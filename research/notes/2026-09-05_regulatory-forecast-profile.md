...
**Compiled:** 2026-09-05; **probability mechanics corrected and every number re-run 2026-09-06** (see section 0). Author: Krishang, with Claude Code. Outputs: `data/processed/abnb_regulatory_events.csv`, `abnb_regulatory_profile.csv`, `abnb_regulatory_contributions.csv`; figure `analysis/figures/abnb_regulatory_profile.png`.
...
## 0. Correction, 6 September 2026 (WS22) — probability mechanics repaired, numbers below are the re-run

An audit (`docs/2026-09-06_audit_findings_ai_handoff.md`, finding A06) found three defects in how
`abnb_regulatory_forecast.py` sampled the event table. They were mechanics, not assumptions: no probability,
loss range, correlation or exposure anchor in the register was changed. **Every number in this note has been
regenerated from the repaired simulator on 6 September 2026** (same seed, 200,000 draws). The pre-fix numbers are
reproducible from `analysis/src/overnight/22_regulatory_before.py` and stored in
`data/processed/overnight/22_regulatory_before.csv`; the before/after comparison is
`data/processed/overnight/22_regulatory_delta.csv`; the acceptance checks are
`analysis/src/overnight/22_regulatory_checks.py` and `22_regulatory_checks.csv` (77 checks, 0 failures).
Full write-up: `research/notes/overnight/22_regulatory-probability-fix.md`.

| Defect | What the old code did | What it now does |
|---|---|---|
| A06a EU tail without its prerequisite | `EU-TAIL` was described as conditional on `EU-AHA` but drawn from its own correlated uniform, so the binding-caps tail fired **without the act existing** in 1.08% of draws by 2027 (54% of all tail occurrences) and 1.73% by 2030 (22% of tail occurrences) | `EU-TAIL` is nested inside `EU-AHA` (fires on the parent's most extreme draws). The stated 2% / 8% are kept as **marginal** probabilities, which is what the p27/p30 column and this note's prose always claimed; the implied conditional escalation rate is 17% by 2027 (0.02/0.12) and 18% by 2030 (0.08/0.45) |
| A06b Barcelona partial branch | Rationale said 0.55 x (1 − 0.45) = 30.25% effective, but the partial branch used its own correlated uniform gated on the full branch, delivering **23.05%** | the two branches are one ordinal ladder on a single Barcelona uniform: full 45%, else partial at conditional 55%, so the effective probability is exactly **30.25%** and overlap is impossible by construction |
| A06c 2027 and 2030 were not one path | residual draws were renewed per horizon, so 3.01% of draws had the EU act in force by 2027 and absent by 2030, and **72% of draws reversed at least one event** | one uniform and one loss-size draw per event per trial, thresholded at p27 and p30. No event reverses (the table names no reversal state) and 2027's event set is a subset of 2030's on every path. The script now asserts p30 ≥ p27 |

**Effect on the headline numbers** (revenue loss, % of global revenue; compliance cost and one-off cash are
separate line items and barely move):

| | 2027 median | 2027 mean | 2027 p95 | 2030 median | 2030 mean | 2030 p95 |
|---|---|---|---|---|---|---|
| Before | 0.45% | 0.75% | 2.74% | 1.71% | 2.20% | 6.38% |
| After | 0.45% | 0.75% | 2.67% | 1.66% | 2.20% | 6.69% |

The centre of the distribution is unchanged; the shape of the upper tail is not. The 2027 90th percentile falls
from 1.95% to 1.79% and the 99th rises from 5.44% to 6.28%, because the tail scenario no longer occurs on its own
— when it happens it now sits on top of the parent act's loss, so that mass moves out of the p90 region and into
the far tail. At 2030 the 95th rises from 6.38% to 6.69% for the same reason, and the median falls 4bp because
the orphan-tail draws are gone. `BCN-PARTIAL` expected loss rises from 0.025% to 0.033% of revenue. In per-share
terms (22x EV/EBITDA, 567M shares) the 2030 median moves from $8.44 to $8.25 and the 2030 95th from $28.71 to
$30.04. **Nothing in the investment conclusion changes**; the tail is now honest about being conditional.

What this does **not** fix: these are still single-analyst scenario probabilities with no external calibration
(section 5). Repairing the mechanics does not make them empirical.
...
|---|---|---|
| Incremental net revenue loss, median | 0.45% of revenue | 1.66% |
| Mean | 0.75% | 2.20% |
| 25th to 75th percentile | 0.20% to 0.84% | 0.89% to 3.02% |
| 90th / 95th / 99th percentile | 1.79% / 2.67% / 6.28% | 4.15% / 6.69% / 8.35% |
| P(loss over 1%) / P(over 2%) | 18.9% / 8.9% | 71.0% / 43.1% |
| Recurring compliance and mitigation cost (EBITDA), median | 0.13% of revenue | 0.16% |
| Total EBITDA hit at 70% contribution margin, median (mean) | $72M ($104M) | $213M ($272M) |
| Value per share at 22x EV/EBITDA, median (mean) | $2.78 ($4.04) | $8.25 ($10.54) |
| One-off cash (Spain fine, Chicago), expected | $50M | $65M |

Three separate line items, never netted: **revenue loss** (the run-rate rows above), the **recurring compliance
and mitigation cost** which is an EBITDA expense and not lost revenue, and **one-off cash** (Spain's fine,
Chicago) which touches neither run-rate line.

Against the $181.94 close (4 Sep 2026), the median regulatory drag is about 1.5% of value by 2027 and 4.5% by 2030; the 95th percentile is 7% and 17%. Regulation is a valuation haircut and a growth-rate tax on Europe, not an existential risk within the pitch horizon. The distribution is right-skewed and bimodal at 2030: a "local rules keep grinding" mode near 1% and an "EU framework in force" mode near 3%, with 11% of mass past 4% and 1.6% past 8%.

Three findings matter for the pitch:

1. **One event carries the tail.** The EU Affordable Housing Act (presentation 9 September, adoption most likely 2028) is 45% likely to be in force with enforcement by 2030 and accounts for 34% of the variance in the 2030 distribution on its own, and 80% together with its binding-caps tail (which, since the 6 Sep correction, can only happen if the act itself happens). Everything else in the register sums to a median 1.23% drag by 2030 (95th percentile 2.08%) with the EU act switched off. The prediction card for 9 September: if the proposal excludes primary residences and frames restrictions as proportionate local measures (as the leaked draft does), hold the number; if it contains binding quantitative caps or applies to primary residences, move EU-AHA's 2030 probability to 60% and the tail to 15%.
...
| EU tail: binding caps, fast enforcement in FR/ES/IT/PT/NL (marginal; nested inside the row above, so conditional on it 17% by 2027 and 18% by 2030) | 2% | 8% | 2.0 / 3.5 / 6.0 | 0.31% | as above; competence objections |
...
| Barcelona partial (30% to 70%, or grandfathering past 2029) | 0% | 30.25% effective (55% conditional on no full phase-out) | 0.05 / 0.10 / 0.18 | 0.03% | same |
...
Correlation: EU events share a latent "European housing politics" factor (copula rho 0.45); US events a weaker one (0.25). Without the correlation the 2030 90th percentile would be 3.85% instead of 4.15% and the 95th 6.18% instead of 6.69%. Two dependencies are logical rather than statistical and are enforced as such since 6 Sep 2026: the EU tail cannot occur without the EU act, and Barcelona's full and partial phase-outs are rungs of one ladder.
...
**Distribution**: see section 1; mass below 0%: 2.8% (2027) and 0.4% (2030), from NYC loosening; mass above 8%: 1.6% (2030).
**Sensitivity** (re-run 6 Sep 2026 on the corrected sampler, 200k draws): EU-AHA 2030 probability 45% to 65% with the tail at 15% moves the 2030 median from 1.66% to 2.44% and the 90th percentile from 4.15% to 6.52%; EU conditional loss mode 1.2% to 2.0% moves the 2030 mean from 2.20% to 2.32% (median 1.69%); scaling every conditional loss by 1.5x (less recapture) moves the 2030 median to 2.50% and the 95th percentile to 10.03%; removing the EU act entirely leaves a 2030 median of 1.23% and a 95th percentile of 2.08%; removing the correlation moves the 2030 90th percentile to 3.85%. A further construction check: replacing the nested "severity ladder" for the EU tail with an independent Bernoulli gate inside the parent (same 2%/8% marginal) moves the 2030 95th percentile from 6.69% to 6.44% and the median from 1.66% to 1.67%, so the conclusion does not hinge on which admissible nesting is used (`22_regulatory_checks.csv`, check 5).
...
- Horizon losses are run-rates, not cumulative; timing within the horizon is not modelled. Since 6 Sep 2026 the two horizons are one path per trial (an event in force by end-2027 is in force by end-2030), so the 2027 and 2030 columns can be read as a scenario evolving, not as two unrelated draws. There is no reversal state: an event that is in force cannot lapse. That is a modelling choice, and it is why `PT-RETIGHT`-style reversals appear only as a *lower probability* of the event, never as an un-doing of it.
- The Monte Carlo is 200,000 draws, so probabilities are accurate to about +/- 0.2pp and the reported percentiles to roughly the second decimal; differences smaller than that between the before and after tables in section 0 are sampling noise, not effects.
