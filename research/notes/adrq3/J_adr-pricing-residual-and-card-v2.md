...
- **Does not redo:** party size, length of stay and geographic mix (workstream I), the FX estimators (WS-B, carried through H), the H reconstruction of the residual itself.
...
4. **Card v2, on the H card's mix inputs because workstream I's summary file was not present when J3 ran: 3Q26 reported ADR +3.4%, $177.2, central band $174.8 to $179.6 (+2.0% to +4.9%); 4Q26 +4.0%, $174.3, band $171.9 to $176.6 (+2.6% to +5.4%).** Ex-FX +3.9% in both quarters (band +2.5 to +5.3): residual 4.61 + geographic mix -1.19 + unit size +0.74 + LOS +0.30 + new business -0.48 + interaction -0.10. FX at the midpoint of the two H estimators (-0.43 pp in 3Q26, +0.15 in 4Q26); the estimator choice alone spans +2.8% to +4.1% in 3Q26 and +3.2% to +4.9% in 4Q26. Against the H card (+3.2% and +3.9%) v2 is 0.3 pp higher in 3Q26 and 0.1 pp higher in 4Q26, entirely because the residual moved from H's trailing-four-quarter 3.94 to the persistence 4.61, less the fee-migration increment H added and v2 carries as a memo. At the team nights baseline this is GBV $26.0bn (+13.7%) in 3Q26 and $23.1bn (+13.3%) in 4Q26. J3 re-runs in one command and reads `I_mix_terms_3q26.csv` if it exists, so the card refreshes to I's measured terms without edits; `mix_source` in every output row says which inputs were used.
...
Mix inputs: H card table 2.3 (`mix_source` = "H card inputs (I_mix_terms_3q26.csv not present when J3 ran)"). Re-run `py -3.13 analysis/src/adrq3/J3_residual_nowcast_card_v2.py` once I's file exists.
...
| Geographic mix | -1.64 / -1.19 / -1.04 | -1.65 / -1.19 / -1.02 | H (WS-C C5, FY25 shares) | sourced |
| Unit size (party size) | 0.00 / 0.74 / 0.98 | same | H (13_party_size forecast) | descriptive |
| Length-of-stay mix | 0.00 / 0.30 / 0.45 | same | H (14a-c) | descriptive |
...
| **Ex-FX ADR** | **2.48 / 3.88 / 5.28** | **2.48 / 3.88 / 5.28** | root-sum-square half range 1.40; arithmetic extremes -0.14 to 6.38 | |
...
| **Reported ADR y/y, point (midpoint FX)** | **+3.45%** | **+4.03%** |
| Reported y/y, EUR fit / baskets | +2.76% / +4.14% | +3.22% / +4.85% |
| Reported y/y, central band | +2.05 to +4.85 | +2.63 to +5.43 |
| Reported y/y, wide band (arithmetic extremes plus FX spread) | -1.26 to +6.64 | -0.80 to +7.35 |
| **ADR, $ point** | **$177.20** | **$174.26** |
| ADR, $ central band | $174.81 to $179.60 | $171.92 to $176.61 |
| GBV at 146.8mm / 132.7mm nights | $26.01bn, +13.7% | $23.12bn, +13.3% |
| Revenue at H's same-quarter take rate (17.88% / 13.62%) | $4,651mm, +13.6% | $3,150mm, +13.4% |
...
4. **Score the pre-registered rule on 5 November.** The v2 persistence residual implies ex-FX of 3.9 pp with H's mix, naive says 4.0; the print distinguishes neither from the other, so the test that matters is the residual's own level, not the ex-FX total.
