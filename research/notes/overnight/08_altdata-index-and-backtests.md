...
| Macro/peer components | 2022Q1+ | 12 | 5 | 12 | 3 | 3 | 1 | 0.68 |
...
The blocker is composition. The cross-city median rests on 1 city (Rome) for 2023Q4-2024Q4, 2-5 cities to 2026Q1, 9 in 2026Q2 and 13 in 2026Q3. The fixed-13-city variant therefore has **n = 0** quarters with a year-ago comparison, exactly as note 03 predicted. On the chained series, nothing is flagged (0 of 36) and the best walk-forward is `reviews_l30d` matched y/y at 0.81 with r +0.29 on n = 11. Listings y/y is actively harmful (3.63 vs naive) — it is a supply and regulation series (Barcelona, Spain delistings), not demand. Like-for-like price ends 2025Q3 because Inside Airbnb dropped listed prices; **r +0.03 with ADR y/y on n = 8** (was +0.13 before the WS21 pair-eligibility fix). The series now uses `price_pair_eligible` **and** `pair_eligible_pit`, so a pair whose endpoint was a partial Inside Airbnb scrape no longer enters: **2025Q3 moves -5.71% (3 cities) to -8.35% (2 cities)** as the partial Sep-2025 Austin dump drops, and 2024Q4 stays 0.00% but on 1 city, not 2. The n = 8 rests on 1-4 cities per quarter, two of the eight quarters on a single city. The verdict is unchanged: it forecasts nothing.
...
- Common Crawl survival: appears in the beat-naive list (0.68 vs nights) on n = 10 with r -0.54 (after WS19's audit-A05 fix to the y/y alignment; it was 0.69 on n = 11 with r -0.58) — wrong sign for a demand read (better survival, slower nights) and it is an annual-ish supply-quality series. Discard; note 03 reached the same conclusion by a different route.
...
| Nights y/y | demand index ex Inside Airbnb | +10.0% | 2.8 | 10.3 | 8.8 |

(The two demand-index rows moved by 0.0-0.3 pp on 7 Sep 2026 when `pr_hotel_revpar_yoy` became point-in-time; the ex-Inside-Airbnb index went +10.2% -> +10.0%. The composite indexes still lose to naive on every target, so neither row is a forecast.)
...
- This note **does not** contradict note 03 on hotel RevPAR and BEA. Those pairs beat naive here because 2024-26 nights growth is nearly flat, not because they carry information: their correlation with nights falls from +0.87 (2023Q1+) to +0.46 and +0.33 (2024Q1+).
- **Correction, 7 Sep 2026 (WS20 section 8, applied in WS26).** `pr_hotel_revpar_yoy` averaged Marriott and Hilton RevPAR unconditionally, but MAR reported *after* ABNB in 2023Q3 (-1 day) and 2025Q1 (-5 days) and HLT in 2024Q2 (-1 day), so at 3 of 23 prints the feature used a number that did not exist yet while the registry called it "available before print". It now averages only peers with `lead_days > 0` (the panel carries `pr_hotel_peers_reported_before_print`). Effect on `pr_hotel_revpar_yoy -> nights_yoy`: full window n 18 -> **17**, r +0.941 -> **+0.921**, walk-forward vs naive 0.836 -> **0.943**, vs AR(1) 0.605 -> **0.698**, sign accuracy 10/14 -> **8/13**; post-2022 window n 14 unchanged, r +0.879 -> **+0.869**, vs naive 0.676 -> **0.702**. The composite demand indexes still lose to naive on every target, and the family counts are unchanged (598 tests, 29 beat naive, 7 by 20% or more).
