...
3. **Host churn is high and rising, but it is churn out of the tail, not out of the business.** Inside Airbnb year-ago retention across the six cities with a clean 2025 and 2026 read **ex-Austin** fell from **75.5% to 73.4%** (-2.1 pts), with new listings replacing them at a flat **25.4% → 25.4%** of the ending base. *(Corrected 7 Sep 2026: the earlier 75.4% → 71.1% was a seven-city mean dominated by Austin's permanent listing-count step, which is a scope reduction, not churn — WS21 / audit A04.)* Common Crawl re-fetch survival is flatter (90% in 2022, 85-88% since). Roughly a quarter of listings turn over every year and always have; the platform grows because gross adds exceed exits, and matched-listing review volumes are still rising (+4 to +13% y/y in 2026 in every city except Austin, roughly flat, and Paris, +1%).
...
5. **Regulation is a ~0.15% revenue drag in 2026 rising to ~0.86% in 2028 at the median, and it is 93% European.** In nights terms that is roughly **-1.1% on EMEA nights in 2027 and -2.0% in 2028**, versus -0.08% and -0.15% on North America. The tail is what matters: the 95th percentile is 2.7% of revenue in 2027 and 4.0% in 2028, and **77.9%** of the variance is two EU items (EU-AHA and the EU tail). *(Numbers refreshed 6-7 Sep 2026 after WS22's audit-A06 repair; see section 5.)*
...
**Corrected 7 Sep 2026 (WS21 / audit A04, applied in WS26).** On the seven cities with a clean read in both years the raw figure was 75.4% → 71.1%, but that seven-city mean is dominated by **Austin**, whose 2026 pairs straddle a permanent listing-count step rather than measuring churn. **Excluding Austin, the six-city figure is 75.5% → 73.4% (-2.1 pts; n = 14 pairs in 2025, 34 in 2026), and the new-listing share of the ending base is 25.4% → 25.4%, exactly flat.** The "retention is falling" claim survives but is **half the size**, and the new-listing share is flat rather than drifting up. Source: `inside_airbnb_like_for_like.csv` filtered on `pair_type=year_ago` **and the published `pair_eligible` flag** (25 of 103 pairs excluded, mean retention 0.489 against 0.726 for the 78 clean pairs); summary rows `inside_airbnb_year_ago_retention_six_city_ex_austin` in `11_supply_economics.csv`.

**Austin is a scope step, not churn — corrected.** The original text called Austin "a genuine outlier, not an artefact" on the grounds that both endpoints of every 2026 pair are full-scope dumps. That is true of the coverage flag and still misleading: Austin stepped from 15,187 listings in Jun 2025 to ~11,000 from Sep 2025 and has stayed there, so every 2026 year-ago pair spans the step and reads 49-53% "retention" mechanically. WS21 marks all four with `span_step_warning`, and the `inside_airbnb_year_ago_retention_austin` 2026 row (0.509) now carries a SCOPE ARTEFACT label in `11_supply_economics.csv`. **Austin has no usable year-ago retention or review y/y until two post-Sep-2025 Septembers exist (Sep 2026); do not put the 0.509 on a slide.** The reviewed-in-LTM share rising from 0.60 to 0.73 is still consistent with an inactive-listing purge, but it is not evidence the 500,000-listing removal programme is visible in Austin's retention.
...
|---|---|---|---|---|---|---|---|
| 2026 | 0.150% | 0.251% | 0.891% | 93% | **-0.36%** | -0.03% | **-0.15%** |
| 2027 | 0.450% | 0.752% | 2.672% | 93% | **-1.07%** | -0.08% | **-0.45%** |
| 2028 | 0.855% | 1.235% | 4.011% | 93% | **-2.03%** | -0.15% | **-0.855%** |

Output: `11_regulatory_overlay.csv`, regenerated 6 Sep 2026 after **WS22's repair of the Monte Carlo's conditional dependencies (audit A06)**: the EU binding-caps tail can no longer fire without the EU act, Barcelona's partial phase-out now carries the 30.25% the register always stated, and the 2027 and 2030 columns are one path rather than two independent draws. No probability, loss range or exposure anchor changed. The mean exceeds the median by 60-70% because the distribution is right-skewed: **P(revenue loss >1%) is 18.9% by 2027 and 71.0% by 2030** (was 19.5% / 70.7% pre-fix); P(>2%) is 8.9% and 43.1%. Two events carry **77.9%** of the 2027 variance — EU-AHA (p=12% by 2027, 1.53% loss if in force) and the EU tail (p=2%, 3.81% loss). *(The "68%" in the original text was a transcription error: the pre-fix contributions file gave 38.2% + 37.0% = 75.2%.)* The tail is genuinely fatter once properly conditional: the 2030 95th percentile goes 6.4% of revenue / $28.71 per share to **6.7% / $30.04**.
...
| **Regulatory nights drag, EMEA** | -0.36% (2026), **-1.07% (2027)**, -2.03% (2028), median | % of EMEA nights | Subtract from the EMEA nights build | `11_regulatory_overlay.csv` |
| **Regulatory nights drag, NA** | -0.03% / -0.08% / -0.15% (2026/27/28), median | % of NA nights | Subtract from the NA nights build | same |
| **Regulatory revenue drag, global** | median 0.150 / 0.450 / 0.855%; **mean 0.251 / 0.752 / 1.235%**; p95 0.891 / 2.672 / 4.011% (2026/27/28) | % of revenue | Use the **mean** in the base case, p95 in the bear | same |
...
| **Host churn** | Year-ago listing retention **75.5% (2025) → 73.4% (2026)** on six cities ex-Austin; gross adds a flat 25.4% of the ending base | share | Background; no model line, but it is the answer to "is the supply base eroding" (no) | `11_supply_economics.csv` |
