...
- **Other inputs:** `09_ff_factors_daily.csv` (Ken French 5 factors + momentum, through 31 Jul 2026); `09_analyst_actions.csv` (466 sell-side actions, yfinance/Benzinga feed); `09_short_interest.csv` (84 semi-monthly settlements, marketbeat + nasdaq.com, Feb 2023 to 14 Aug 2026); `data/processed/abnb_earnings_reactions.csv` (23 prints); `abnb_major_moves_events.csv` (41 moves ≥7%); `predictive/02_peer_prints.csv` (BKNG/EXPE/MAR/HLT print dates); `citadel-abnb/research/regulatory/factors.json` (32 dated regulatory factors); `data/processed/abnb_options_ledger.csv` (copied 6 Sep 2026 from the main tree, then regenerated on schema v2 by workstream 23) and `data/processed/overnight/23_options_event_estimates.csv`; BLS release-date archives (`bls.gov/schedule/<year>/home.htm`, fetched 6 Sep 2026); a live yfinance ABNB option chain pulled 6 Sep 2026; fintel.io 13F snapshot.
- **Outputs:** 24 CSVs `data/processed/overnight/09_*.csv`, 6 figures `analysis/figures/overnight/09_*.png`.
- **Correction, 6 Sep 2026 (workstream 23, audit finding A08):** the options block - bottom-line item 8, §7, correction 10.4 and the options bullet in §11 - has been rewritten. The event-implied-move estimator was mis-identified and its "implied event premium = 0.002 pts" result is **withdrawn**. `09_implied_move_live.json` and the `live_*` columns of `09_implied_vs_realised.csv` were regenerated on the corrected method. Nothing else in this note changes. Details: `research/notes/overnight/23_options-estimator-fix.md`.
...
8. **The option market's 5 Nov event premium is not measurable today — and the earlier "premium is zero" reading was an artefact.** *(Corrected 6 Sep 2026 by workstream 23, audit finding A08; see `research/notes/overnight/23_options-estimator-fix.md`.)* The old √time test compared the 20 Nov and 18 Dec straddles — **both of which contain the print** — so it could never have detected an event premium; √time consistency between two post-event expiries is what you see whether or not the event is priced. Fitting the proper model `sigma_i^2*T_i = b*T_i + E*d_i` on the live 6 Sep chain gives an event standard deviation of **2.3% (23 Oct vs 20 Nov pair)**, **6.7% (all 10 maturities)** and **non-positive (20 Nov vs 18 Dec pair)** — a spread wider than the answer, because the pre-event weekly IVs themselves range over 6.7 vol points. **No event-implied move can be quoted today.** The historical base rate is the useful number: mean absolute day-1 move **7.1%**, median **6.9%**, and 10 of 23 prints moved ≥8%.
...
**The live term structure: the event premium is not identified today.** *(This paragraph was rewritten on 6 Sep 2026 by workstream 23 to close audit finding A08. The superseded version claimed "the implied event premium is 0.002 pts"; that came from comparing the 20 Nov and 18 Dec straddles, both of which contain the print, and from `abnb_options_ledger.csv`'s `event_implied_move_pct`, which was computed as `sigma_near^2*T_near - sigma_far^2*T_near` and therefore equals `E*(1 - T_near/T_far)`, not E. Sources for everything below: `analysis/src/abnb_options_ledger.py`, `data/processed/abnb_options_ledger.csv`, `data/processed/overnight/23_options_event_estimates.csv`, `09_implied_move_live.json`, all regenerated 6 Sep 2026 19:20 UTC.)*

The estimator now fits an explicit total-variance model, `sigma_i^2 * T_i = b * T_i + E * d_i`, where `d_i = 1` if expiry *i* settles strictly after the latest plausible release timestamp. The ABNB event window is **4-12 Nov 2026, third-party estimate, not company-confirmed** (Yahoo calendar says 5 Nov; Airbnb IR had not posted a date as of 6 Sep). **The 6 Nov weekly is not listed yet** - the listed expiries jump from 23 Oct straight to 20 Nov - so the nearest post-event expiry sits 15 days after the print.

Raw measures on the live 6 Sep chain (verified same-strike straddles; these are **total** implied moves to expiry, not event moves):

| Expiry | Class | dte | Strike | ATM IV | Straddle mid, % of spot | Straddle bid-ask, % of spot | Quote quality |
|---|---|---|---|---|---|---|---|
| 16 Oct 2026 | pre-event | 40 | 180 | 32.72% | 8.20% | 7.67-8.74 | fair |
| 23 Oct 2026 | pre-event | 47 | 185 | 37.81% | 9.11% | 7.12-11.10 | poor |
| 20 Nov 2026 | post-event | 75 | 180 | 38.15% | 13.38% | 12.94-13.82 | fair |
| 18 Dec 2026 | post-event | 103 | 180 | 38.68% | 15.65% | 14.92-16.38 | fair |
| 15 Jan 2027 | post-event | 131 | 180 | 38.22% | 17.01% | 15.80-18.22 | fair |

Fitted event estimates, all three identified specifications:

| Specification | Maturities | Background vol b | Event variance E | Event sigma | Expected abs move |
|---|---|---|---|---|---|
| 23 Oct (pre) + 20 Nov (post) | 2 | 37.81% | 0.000534 | **2.31%** | 1.84% |
| All eligible (7 pre + 3 post) | 10 | 36.21% | 0.004524 | **6.73%** | 5.37% |
| 20 Nov + 18 Dec (both post) | 2 | 40.08% | -0.003106 | **non-positive** | - |

**These three answers are not reconcilable, and that is the finding.** The seven pre-event weekly ATM IVs span 31.1% to 37.8% - 6.7 vol points of dispersion with no scheduled event between them, i.e. pure measurement noise in the background baseline. Holding the 20 Nov quote fixed and sweeping the assumed background across exactly that observed range moves the implied event sigma from **2.3% to 10.0%** (`identification_diagnostics.E_sensitivity_to_assumed_background` in `09_implied_move_live.json`). The leave-one-maturity-out range on the 10-maturity fit is 5.2-8.1%. **E is inside the noise.**

What can and cannot be said today: (i) the raw straddles above are real, quoted, verified same-strike prices and can be quoted as total implied moves to expiry; (ii) no event-implied move can be quoted; (iii) the earlier claim that the market prices *no* event premium is withdrawn - a non-positive fit on two post-event expiries is a statement about the background term structure, not about the earnings premium. **Re-run `analysis/src/abnb_options_ledger.py` in the week of 26-30 Oct 2026**, once the 6 Nov weekly is listed; a post-event expiry one day after the print collapses the background-baseline sensitivity and turns E into a usable number to compare against the 7.1% base rate.
...
4. ~~`data/processed/abnb_options_ledger.csv` exists only in the main tree ... the correct answer at 76 dte does happen to be approximately zero.~~ **Superseded 6 Sep 2026 (workstream 23, audit finding A08).** The ledger and its script have been copied into the overnight tree at the same relative paths and regenerated. The old `event_implied_move_pct` was not a failed calculation returning a real zero - it was the wrong estimand (it computes `E*(1 - T_near/T_far)`) computed on two expiries that both contain the print, so it is withdrawn rather than corrected in place. The 5 Sep 2026 rows are retained in the new file as `schema_version = 1` legacy rows with the event column removed and `qq_flags = legacy_schema_v1;event_estimate_withdrawn_A08`. The claim in §7 that "the correct answer at 76 dte is approximately zero" is also withdrawn: see the rewritten §7 and `research/notes/overnight/23_options-estimator-fix.md`.
...
- **Options: no event-implied move is quotable yet** *(corrected 6 Sep 2026, WS23 / A08)*. The listed chain skips from 23 Oct to 20 Nov, so the nearest post-event expiry is 15 days after the print, and the fitted event sigma ranges from non-positive to 6.7% depending on specification - wider than the answer. Quote only the raw totals: 20 Nov straddle **13.38% of spot** (K 180, mid; bid-ask 12.94-13.82), 18 Dec **15.65%**. Re-run `analysis/src/abnb_options_ledger.py` in the week of **26-30 Oct 2026** once the 6 Nov weekly lists: if the event-implied move then comes in below ~7% the straddle is cheap against the base rate, above ~9% it is rich. That is still the cheapest concrete trade-structure input we have - it is just not available today.
...
1. **Re-run the options block in the week of 26-30 Oct 2026** once the 6 Nov weekly is listed: `py -3.13 analysis/src/abnb_options_ledger.py` then `py -3.13 analysis/src/overnight/23_update_ws09_options.py`. That converts §7 from "not identified" into an actual implied-vs-base-rate comparison and gives the card a trade structure. Re-run the acceptance tests first: `py -3.13 analysis/src/overnight/23_options_estimator_test.py --dry-run`.
