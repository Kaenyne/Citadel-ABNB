...
These are proposed research milestones, not scheduled automations. No new earnings outcome arrives before the stated submission deadline; the next scheduled print is5November. Lock prospective forecasts now for later learning, but do not imply that waiting for that print can validate the preliminary memo in time.
