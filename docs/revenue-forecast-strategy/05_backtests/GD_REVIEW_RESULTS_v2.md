# GD-REVIEW — independent GBV decision review, corrected scorer convention
...
The new horizon comparison holds four models to identical rows. Dollar errors below are **raw issued-guide-midpoint RMSE, $M**. Direct inspection of frozen harness/score.py confirms its error is also point minus actual; it does not implement the required ±$0.5M letter-rounding interval. The parent is adding a separate interval sensitivity and a harness change request without editing frozen code.
...
Canonical code: `analysis/src/forecast_methods/gbv_decision_0915_v1/review_v1/{check_outputs.py,boundary_checks.py,street_checks_v2.py,supplement_checks_v2.py,registration_checks.py}`. Run from the worktree root with `.venv/Scripts/python.exe` from the main repo and a new `--out` directory; complete commands are in `review_v1/README_RESULTS_v2.md`. Receipts under the corresponding data folder: `horizon_v1`, `boundary_v1`, `street_v2`, `supplement_v2`, `registration_v1`. They bind exact source manifests and review code hashes.
...
Registration is approved for research. The reviewer has not written the shared registry or run its frozen scorers. Frozen score ratios whose baseline joins omit vintage/horizon cannot replace the local same-origin race. Both local and frozen-harness point metrics use raw midpoint error. The repo-required interval convention must be checked separately; do not claim the frozen harness already implements it. No broad model promotion or calibrated predictive interval is implied by registration.
...
## Correction to superseded review v1

The original review v1 incorrectly implied the frozen scorer applied a ±$0.5M interval loss. The parent identified the mismatch and the reviewer inspected frozen harness/score.py directly: it calculates err = point - actual, then raw MAE/RMSE. The source/model arithmetic and all reported raw-error checks remain valid. This new note supersedes v1 on that scoring statement; v1 is preserved, and no frozen scorer was changed. The additive interval sensitivity is pending its own bound receipt and should be linked in the parent's completion note before formal acceptance language. This is an inherited scorer/convention gap, not demonstrated forecast leakage.
