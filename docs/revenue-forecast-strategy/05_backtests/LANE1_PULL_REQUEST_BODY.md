...
Implements and verifies the complete Lane 1 kernel, guide-surprise, term-structure, valuation, regional, GBV-feature, and O-D FX packages. The supported kernel is v2; four exact claims survived all three independent refuters, while the failed and underpowered results remain explicit.

## Why

Establishes reproducible guide and valuation infrastructure while testing whether the supplied data support a historical expectations edge. The evidence supports bounded accounting and data-availability claims; it does not establish historical trading alpha or justify replacing B4.

## Checklist

- [ ] Teammate review: confirm the numbers against the linked model-derived tables and source evidence below.
- [x] Model assumptions were not changed; valuation alternatives and date conventions are disclosed.
- [x] Existing model and deck binaries were not edited; the new valuation page is a separate generated artifact.
- [x] No notebook outputs added.
- [x] Frozen harness/L0 and existing package artifacts preserved; supported package tests and frozen checks passed as detailed below.
- [x] No licensed exports, credentials, or raw stores included.
...
This PR proposes the pushed `codex/lane1-full` branch at `f666523043166938a5b49ba697f5ea6d56059569` for `main` in shared `Kaenyne/Citadel-ABNB`. The latest merge check against `main` at `cde896a274cf6f36eff64461737bf96c6871858f` is clean. The earlier integration-denied creation attempt is preserved in the publication receipt as historical evidence. No direct update to main has been made.
