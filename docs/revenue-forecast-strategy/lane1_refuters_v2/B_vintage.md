...
CLAIM = At strict pre-guide dates, the supplied data yield zero eligible FY-guide-revision tests in W1 (0/14) and W2 (0/10), so the live kernel–consensus gap is a conditional scenario, not validated revision alpha.

LENS = vintage

Package note: docs/revenue-forecast-strategy/05_backtests/ALPHA_B_TERM_STRUCTURE.md. Read only this note, its actual registry files if any, and its named raw inputs. Do not read other package code or derived outputs. Independently recompute the central count, price bridge or statistic relevant to your lens. Write only docs/revenue-forecast-strategy/05_backtests/REFUTE_B_vintage.md. Parent owns board, scorer and git operations. Include at least five numbered attacks, each with explicit claim, attack, result, and evidence. Give one final verdict exactly survived / refuted / partial on the original exact sentence. Partial does not count as survived. Do not substitute a weaker sentence silently. Distinguish arithmetic sensitivity from statistical estimation; if W1/W2 or power are not applicable to a conditional arithmetic identity, explain that boundary instead of inventing statistical support. Record exact commands, runtime if available, and tokens only if available.
