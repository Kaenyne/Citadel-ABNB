# WP-G1 — Point-in-time consensus history from LSEG Workspace into the vintage register

**Lane:** Theo lane · **Effort:** half a day once access exists · **Needs from outside the repo:** a UF-affiliated LSEG Workspace login (self-register at businesslibrary.uflib.ufl.edu/refinitivworkspace). Licensed: exports stay OFF git
...
## The task
In LSEG Workspace export the estimates history for ABNB, NCLH, BKNG, EXPE: revenue (and EBITDA where available) by fiscal period, the
mean / median / number of estimates at every monthly statistics date from Jan 2021 to today. Save under `data/raw/consensus/lseg/`
(gitignored — commit a manifest only). Run `analysis/src/forecast_methods/L0/consensus_history_loader.py` (append-only; back up the register
as `L0_vintage_register.backup_<date>.csv` first) so each statistics date becomes a vintage row with vendor = LSEG.

## Pass line (pre-registered — write it in your note before running)
The register gains vintage-stamped rows at every historical guide date for ABNB; the 6 Aug 2026 Q3 value matches LSEG $4,610M; row counts
before and after are in the note; nothing licensed is committed.

## Outputs
Manifest; loader run log; note `05_backtests/G1_LSEG_VINTAGES.md`. Then tell WP-A / WP-B owners to re-run.
