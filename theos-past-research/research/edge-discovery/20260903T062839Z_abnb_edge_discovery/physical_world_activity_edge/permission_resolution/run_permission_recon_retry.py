"""One escalated retry for each preregistered sandbox-DNS failure."""
...
RESULTS = HERE / "permission_recon_retry_results.csv"
CACHE = HERE / "cache" / "permission_recon_retry"
RATE_SECONDS = 6.2
STOP_HTTP = {401, 403, 429}
CAPTCHA_MARKERS = (b"captcha", b"cf-chl-", b"cloudflare ray id", b"verify you are human", b"robot check")
FIELDS = (
    "recon_id", "retry_of", "source_ids", "requested_url", "effective_safe_url",
    "requested_at_utc", "completed_at_utc", "http_status", "content_type", "bytes",
    "sha256", "cached_body_path", "outcome", "stop_reason",
)
...
def main() -> None:
    ...
    all_rows = list(csv.DictReader(MANIFEST.open(newline="", encoding="utf-8")))
    rows = [row for row in all_rows if row["recon_id"].startswith("PR-PW-R")]
    assert len(rows) == 26
    ...
    for row in rows:
        ...
        retry_of = row["purpose"].split(";", 1)[0].split("=", 1)[1]
        if source_ids and source_ids <= blocked_sources:
            results.append({
                "recon_id": row["recon_id"], "retry_of": retry_of, "source_ids": row["source_ids"],
                "requested_url": row["official_url"], "effective_safe_url": "", "requested_at_utc": "",
                "completed_at_utc": now_utc(), "http_status": "", "content_type": "", "bytes": "",
                "sha256": "", "cached_body_path": "", "outcome": "not_requested_source_stopped",
                "stop_reason": "Prior escalated permission GET for every linked source hit a provider stop condition; no further request.",
            })
            continue
        ...
        if status in STOP_HTTP:
            ...
            outcome, stop_reason = "transport_error", f"{transport_error}; no further retry"
        elif 200 <= status < 300:
        ...
        results.append({
            "recon_id": row["recon_id"], "retry_of": retry_of, "source_ids": row["source_ids"],
            "requested_url": row["official_url"], "effective_safe_url": safe_url(effective_url),
