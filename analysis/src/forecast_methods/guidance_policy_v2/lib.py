"""guidance-policy-v2 (runbook task B2): shared primitives.

COPY of analysis/src/forecast_methods/guidance_policy/lib.py, modified in three
places only: METHOD, OUT and the addition of `normal_from_q10_q50_q90`.  The
...
OUT = REPO / "data" / "processed" / "forecast_methods" / "guidance_policy_v2"
...
# ------------------------------------------------- v2 addition: GBV distribution
def normal_from_q10_q50_q90(q10: float, q50: float, q90: float) -> dict:
    """Fit a NORMAL to a published (q10, q50, q90) ladder and report the skew evidence.

    Returns mu, sd and `asym_pp`, the percentage-point asymmetry of the two halves of
    the ladder.  A skew-normal has three free parameters and exactly three moments to
    match here, so it is only identified by the asymmetry; when `asym_pp` is 0 the
    fitted shape parameter is 0 and the skew-normal collapses to the normal.
    """
    z90 = 1.2815515655446004
    lo, hi = q50 - q10, q90 - q50
    sd = 0.5 * (lo + hi) / z90
    return {"mu": float(q50), "sd": float(sd),
            "sd_from_lower_half": float(lo / z90), "sd_from_upper_half": float(hi / z90),
            "asym_pp": float(100.0 * (hi - lo) / (hi + lo))}
