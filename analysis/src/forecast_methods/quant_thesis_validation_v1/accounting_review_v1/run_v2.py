...
def main():
    ...
    for p in panels:
        ...
        assert same["vendor"] == p["vendor"] and threshold["vendor"] == p["vendor"]
        expected_time = p["observed_timestamp"] if family == "LSEG family" else p["observed_timestamp"][:10]
        assert same["observed_timestamp"] == threshold["observed_timestamp"] == expected_time
        if family != "LSEG family":
            assert same["observation_precision"] == "date only; exact time unavailable"
    ...
    same_numeric_fields = 0
    for fname in ["same_basis.csv", "break_even.csv", "cushion_policy.csv", "stress_surface.csv"]:
        prior, current = read(PKG / "expectations_v1" / fname), read(AUTHOR / fname)
        assert len(prior) == len(current)
        for old, new in zip(prior, current):
            for field in old:
                if field not in new or old[field] == "":
                    continue
                try:
                    left, right = D(old[field]), D(new[field])
                except ArithmeticError:
                    continue
                assert left == right, (fname, field)
                same_numeric_fields += 1
    bindings = {str(p.relative_to(ROOT)): digest(p) for p in [
        ROOT / "analysis/src/forecast_methods/quant_thesis_validation_v1/expectations_bridge_v2.py",
    ...
    receipt = {"reviewer": "/root/uncertainty_auditor", "author": "/root", "status": "PASS_numerical_and_basis_review_timestamp_finding_closed",
               ...
               "bound_inputs_code_and_outputs_sha256": bindings, "forecast_or_investment_adoption": False, "v1_v2_identical_numeric_fields": same_numeric_fields,
               "qualification": "Finding closed in expectations_v2: S&P and Zacks timestamps display date only with explicit missing exact time; all shared numeric fields remain identical to v1."}
