...
def main(source,out):
    ...
    fig.text(.1,.88,'Guide-midpoint error at fixed mid-quarter origins, before the next GBV print',fontsize=11,color='#4d6272')
    for idx,(method,label,color) in enumerate([
            ('candidate_k0_gbv','Retained kernel + forecast GBV','#193e5d'),
            ('B1_guide_growth','Direct guide-growth persistence','#3e918e'),
            ('B2_revenue_naive_cushion','Revenue persistence + cushion','#a8b4bc')]):
    ...
    ax.legend(loc='upper left',bbox_to_anchor=(0,-.20),ncol=1,frameon=False,fontsize=9.5)
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--source',type=Path,default=PKG/'prospective_v1/results_v1/evaluation/scores.csv');p.add_argument('--out',type=Path,default=PKG/'figures_forecast_v1')
