...
async function build(){
 wb=Workbook.create();
 for(const [s,title,rows] of [['Summary','Airbnb financial review',45],['Assumptions','Editable operating and financial assumptions',145],['Revenue','Operating inputs and revenue recognition',35],['Costs','Operating costs and earnings',40],['Cash','Free cash flow, net cash and shares',40],['Valuation','Conditional valuation at 13 September 2027',87],['Case comparison','Scenario results captured from one model',900],['Source','Read-only source inputs and provenance',78]])init(s,title,s==='Case comparison'?'M':'L',rows);
 ...
 V('Assumptions','C4','Case number (1-9)');input('Assumptions','E4',1);
 sheets.Assumptions.getRange('E4').dataValidation={rule:{type:'whole',operator:'between',formula1:1,formula2:9}};
 ...
 for(const [key,r,label,unit] of drivers){
   ...
   qcols.forEach((col,qi)=>{
     ...
     if(['lambda','cushion','adjustment','rev_growth','nights_growth','adr_growth'].includes(key))sheets.Assumptions.getRange(`${col}${r}:${col}${r+data.cases.length}`).setNumberFormat(pct);
   });
 }
 ...
 V('Assumptions','C84','Kernel coefficients are fixed 2/3 and 1/3. Quarter input GBV first affects next-quarter revenue.');
 ...
 V('Assumptions','C133','Valuation date');V('Assumptions','F133',new Date('2027-09-13T00:00:00Z'));sheets.Assumptions.getRange('F133').setNumberFormat('mm/dd/yy');
 ...
 V('Assumptions','C137','Conversion validation complete: free weight failed promotion in W1/W2. Existing K0 seasonal policy retained.');
 V('Assumptions','C138','L3 source verified. FX/RNPL application ineligible: matching pre-hedge baseline and hedge split unresolved.');
 ...
 V('Assumptions','C141','All22 fitted weight and fixed-OLS coefficients are descriptive only; neither replaces operational K0.');

 row('Assumptions',142,'Information date');V('Assumptions','F142',new Date('2026-09-13T00:00:00Z'));
 row('Assumptions',143,'FY26 end');V('Assumptions','F143',new Date('2026-12-31T00:00:00Z'));
 row('Assumptions',144,'FY27 end');V('Assumptions','F144',new Date('2027-12-31T00:00:00Z'));
 row('Assumptions',145,'Exact elapsed FY27 calendar fraction');F('Assumptions','F145','=(F133-F143)/(F144-F143)');
 sheets.Assumptions.getRange('F142:F144').setNumberFormat('dd mmm yyyy');sheets.Assumptions.getRange('F145').setNumberFormat('0.000000');
 V('Assumptions','C146','Uniform FY27 net cash/share flows; full FY27 EBITDA used at the September valuation date.');
 V('Assumptions','C147','Soft/firm combine operating GBV, lambda stress and cushion. No simultaneous net-revenue factor.');
 ...
 V('Source','C46','Revenue: L4 v2 bridge. Immutable L3 bundle 8821961853e4 verified; incremental FX application remains ineligible.');
 ...
 V('Revenue','C29','L3 FX/RNPL application is ineligible. +/-1% is isolated illustrative net after-hedge revenue, not measured FX.');
 ...
 V('Revenue','C32','Free-weight promotion failed W1/W2. Existing fixed 2/3 K0 policy retained; no physical cohort-share claim.');
 ...
 const vl={8:'FY27 adjusted EBITDA',9:'Exit EV / EBITDA',10:'Enterprise value',11:'13 Sep 2027 net cash ex customer funds',12:'Equity value',13:'13 Sep 2027 diluted share proxy',14:'EV/EBITDA value per share',17:'EV / FY27 FCF',18:'P / FY27 SBC-adjusted FCF',19:'P / FY27 earnings proxy',20:'FY28 EV/EBITDA discounted one year',21:'DCF on FCF (legacy forward convention)',23:'31 Dec 2027 comparison value/share',26:'DCF: year',27:'FCF growth',28:'FCF',29:'Discount factor',30:'Discounted FCF',32:'Terminal enterprise value',33:'DCF enterprise value',36:'Legacy reproduction (separate inputs)',38:'Legacy FY27 EBITDA lens',39:'Legacy FY27 FCF lens',40:'Legacy SBC-adjusted FCF lens',41:'Legacy earnings proxy lens',42:'Legacy FY28 EBITDA discounted lens',43:'Legacy DCF on FCF',45:'Legacy six-lens mean'};
 for(const [r,label] of Object.entries(vl))row('Valuation',Number(r),label);
 for(const [cell,formula] of Object.entries({E8:"='Costs'!G25",E9:`=${gr('exit_ev_ebitda')}`,E10:'=E8*E9',E11:"='Cash'!F28+'Assumptions'!F145*('Cash'!G28-'Cash'!F28)",E12:'=SUM(E10:E11)',E13:"='Cash'!F36+'Assumptions'!F145*('Cash'!G36-'Cash'!F36)",E14:'=E12/E13',E17:`=(${gr('exit_ev_fcf')}*'Cash'!G18+'Cash'!G28)/'Cash'!G36`,E18:`=${gr('exit_p_sbcfcf')}*'Cash'!G20/'Cash'!G36`,E19:`=${gr('exit_p_earnings')}*'Cash'!G37`,E20:`=(${gr('exit_ev_ebitda')}*'Costs'!H25+'Cash'!H28)/'Cash'!H36/(1+${gr('cost_of_equity')})`,E21:"=(E33+'Cash'!G28)/'Cash'!G36",E23:"=(E10+'Cash'!G28)/'Cash'!G36"}))F('Valuation',cell,formula);
 ...
 V('Valuation','C57','Main: 13 Sep 2027, exactly 256/365 of FY27 cash/share flows from FY26 end; uniform within-year assumption.');
 ...
 V('Valuation','C60','No arithmetic target average. Rows 17-21 are inherited diagnostic lenses; row 23 is the later December date.');
 ...
 row('Summary',18,'13 Sep 2027 conditional value/share');F('Summary','E18',"='Valuation'!E14");row('Summary',19,'31 Dec 2027 later-date comparison');F('Summary','E19',"='Valuation'!E23");
 ...
 V('Summary','C23','Valuation: 13 Sep 2027. Uniform FY27 cash/share flows; FY27 EBITDA x inherited 16.5x. No adopted target.');
 V('Summary','C24','Q2:27+ and FY28 growth inherited. Free-weight promotion failed; FX/RNPL incremental application ineligible.');
 ...
 // Same-object expectations bridge; the guide diagnostic must never substitute for measured guide expectations.
 band('Summary',28,'Q4 2026 expectations: compare revenue with revenue','H');
 row('Summary',29,'Own conditional revenue R (USD m)');F('Summary','E29',"='Revenue'!H18");
 row('Summary',30,'Yahoo / LSEG-family revenue S (USD m)');V('Source','E67',Number(data.street.revenue_consensus_musd));F('Summary','E30',"='Source'!E67");
 row('Source',67,'Q4 revenue consensus, Yahoo / LSEG family');
 V('Source','C68','Captured 13 Sep 2026 15:20 UTC; not explicit expectations for management guide.');
 row('Summary',31,'Revenue gap R minus S (USD m)');F('Summary','E31','=E29-E30');
 row('Summary',32,'Revenue gap / Street revenue');F('Summary','E32','=E31/E30');sheets.Summary.getRange('E32').setNumberFormat('0.00%');
 row('Summary',33,'Own guide G minus revenue S (diagnostic)');F('Summary','E33','=E20-E30');
 row('Summary',34,'Hypothetical Street guide S / (1+c)');F('Summary','E34',"=E30/(1+'Revenue'!H19)");
 row('Summary',35,'Hypothetical same-cushion guide gap');F('Summary','E35','=E20-E34');
 V('Summary','C37','R-S is like-basis. G-S mixes objects and is NOT a measured guide-expectations surprise.');
 V('Summary','C38','Street guide expectations unavailable. S/(1+c) imposes our cushion; it is not observed consensus.');
 V('Summary','C39','Conditional envelopes are assumed stresses, not CIs; letter-close errors omit forecast GBV/cushion risk.');
 band('Valuation',16,'Inherited diagnostic lenses: 31 Dec 2027','J');
 sheets.Valuation.getRange('E9').setNumberFormat('0.0"x"');
 V('Summary','C36','Revenue consensus vintage: Yahoo / LSEG family, 13 Sep 2026 15:20 UTC; 36 estimates.');
 row('Valuation',62,'Date-only value difference, Dec minus Sep');F('Valuation','E62','=E23-E14');sheets.Valuation.getRange('E62').setNumberFormat(money);
 row('Valuation',63,'FY27 net cash, later-date comparison');F('Valuation','E63',"='Cash'!G28");
 row('Valuation',64,'FY27 shares, later-date comparison');F('Valuation','E64',"='Cash'!G36");
 // Saved case results are a requested capture workflow; input changes visibly invalidate them.
 const metrics=[['Q4:26 guide (USD m)','Revenue','H20'],['FY26 revenue (USD m)','Costs','F8'],['FY27 revenue (USD m)','Costs','G8'],['FY27 EBITDA (USD m)','Costs','G25'],['FY27 net income (USD m)','Costs','G36'],['FY27 FCF (USD m)','Cash','G18'],['13 Sep 2027 net cash (USD m)','Valuation','E11'],['13 Sep 2027 diluted shares (m)','Valuation','E13'],['Enterprise value (USD m)','Valuation','E10'],['Equity value (USD m)','Valuation','E12'],['Value per share','Valuation','E14'],['31 Dec 2027 value/share','Valuation','E23'],['Q4 revenue (USD m)','Revenue','H18'],['R-S revenue gap (USD m)','Summary','E31']];
 for(let ci=0;ci<data.cases.length;ci++)V('Case comparison',`${letters(ci+4)}7`,data.cases[ci].label);
 sheets['Case comparison'].getRange('E7:M7').format={fill:navy,font:{bold:true,color:'#FFFFFF'},wrapText:true,rowHeight:37,columnWidth:22};
 ...
 V('Case comparison','C28','Soft/firm: fixed K0; operating assumptions + lambda +/-0.10pp + cushion; no FX factor. These are not CIs.');
 ...
 V('Case comparison','Z1',JSON.stringify({tracked,metrics,caseCount:data.cases.length}));
 for(const s of Object.keys(sheets)){
   if(s!=='Assumptions'){F(s,'E4',"='Assumptions'!G4");sheets[s].getRange('E4:H4').format.font.color=green;}
 }
 sheets['Case comparison'].getRange('Z1').format.font={size:1,color:'#FFFFFF'};
...
async function capture(metrics){
 const caseCount=data?data.cases.length:JSON.parse(sheets['Case comparison'].getRange('Z1').values[0][0]).caseCount;
 ...
 sheets['Case comparison'].getRange('E18:M19').setNumberFormat(money);
}
async function verify(metrics){
 const diffs=[];
 for(let ci=0;ci<data.cases.length;ci++){
  const c=data.cases[ci],a=c.annual,v=c.valuation;
  const expected=[c.quarters[3].guide,a[0].revenue,a[1].revenue,a[1].adj_ebitda,a[1].net_income,a[1].fcf,v.horizon_net_cash_musd,v.horizon_shares_m,v.enterprise_value_musd,v.equity_value_musd,v.value_per_share,v.december_value_per_share,c.quarters[3].revenue,c.quarters[3].revenue-Number(data.street.revenue_consensus_musd)];
 ...
 if(Math.abs(sheets.Assumptions.getRange('F145').values[0][0]-256/365)>1e-12)throw Error('Calendar fraction failed');
...
async function finish(){
 ...
 console.log(JSON.stringify({workbook:file,sheets:8,status:data?'Recalculated, independently tied, input perturbations verified':'Recaptured all cases and restored the selected case; original-input numerical tie-out is not applicable to edited inputs'}));
}
if(args[0]==='--refresh'){
 ...
 const meta=JSON.parse(sheets['Case comparison'].getRange('Z1').values[0][0]);tracked=meta.tracked;
