...
CASES=['review_with_k','review_without_k','k0_conditional','nights_case_a','adr_mean_reversion',
       'review_with_k_fx_down_sensitivity','review_with_k_fx_up_sensitivity','joint_soft','joint_firm']
LABELS=['Review with fee mechanics','Review without fee mechanics','K0 conditional GBV',
        ...
        'Conditional soft envelope','Conditional firm envelope']
YEARS=[2026,2027,2028]
QKEYS=[f'{y}Q{q}' for y in [2026,2027] for q in range(1,5)]
INFO_DATE=date(2026,9,13)
HORIZON=date(2027,9,13)
FY26_END=date(2026,12,31)
FY27_END=date(2027,12,31)
HORIZON_FRACTION=(HORIZON-FY26_END).days/(FY27_END-FY26_END).days
def read_csv(p):
...
def replicate_legacy():
    ...
    return {'method':'Independent equations on published annual CSV; input precision four decimals',
            ...
            'new_target_convention':'13 September 2027; 256/365 of FY27 cash/share flows; no adopted target'}
...
def value(fin,g):
    prior,a,b=fin
    ev=g['exit_ev_ebitda']*a['adj_ebitda']
    hc=prior['net_cash']+HORIZON_FRACTION*(a['net_cash']-prior['net_cash'])
    hs=prior['shares']+HORIZON_FRACTION*(a['shares']-prior['shares'])
    eq=ev+hc;dec_eq=ev+a['net_cash'];dec_value=dec_eq/a['shares']
    lenses=[dec_value,(g['exit_ev_fcf']*a['fcf']+a['net_cash'])/a['shares'],g['exit_p_sbcfcf']*a['sbc_adj_fcf']/a['shares'],
    ...
    # Other lenses remain diagnostic legacy convention; never mechanically averaged into a target.
    return dict(enterprise_value_musd=ev,equity_value_musd=eq,value_per_share=eq/hs,
      horizon_date=HORIZON.isoformat(),horizon_fraction=HORIZON_FRACTION,horizon_net_cash_musd=hc,horizon_shares_m=hs,
      december_equity_value_musd=dec_eq,december_value_per_share=dec_value,
      date_only_value_difference=dec_value-eq/hs,lenses=lenses)
def prepare(revenue_dir):
    replication=replicate_legacy() # MUST pass before any new model calculation.
    expectations=read_csv(revenue_dir/'expectations_comparison.csv')
    street_rows=[r for r in expectations if r['scenario']=='review_with_k' and r['quarter']=='2026Q4' and r['vendor_family']=='LSEG family' and r['consensus_as_of_timestamp']=='2026-09-13T15:20Z']
    if len(street_rows)!=1:raise ValueError('Need one exact dated Yahoo / LSEG-family revenue comparator')
    street=street_rows[0]
    ...
    joint=read_csv(revenue_dir/'joint_scenarios.csv')
    for r in joint:
        if r['scenario'] not in ['joint_soft','joint_firm']:continue
        forecasts.append({**r,'revenue_musd':float(r['revenue_musd'])/float(r['net_revenue_factor'])})
    ...
    for case,label in zip(CASES,LABELS):
        ...
        operating_case={'joint_soft':'adr_mean_reversion','joint_firm':'nights_case_a'}.get(case,source_case)
        if case.startswith('joint_'):
            adjustment=float(next(r['net_revenue_factor'] for r in joint if r['scenario']==case))-1
        quarters=[]
        for q in QKEYS:
        ...
        cases.append(dict(scenario=case,label=label,source_scenario=source_case,operating_scenario=operating_case,adjustment=adjustment,quarters=quarters,annual=annual,valuation=valuation))
    return dict(as_of='2026-09-13',case_names=CASES,replication=replication,inputs=base_inputs,globals=globals_,fy25=m.FY25,h1=m.H1_26,
      ...
      fx_status='L3 source verified; application ineligible: matching pre-hedge baseline and hedge split unresolved. Incremental FX/RNPL is unestimated, not zero.',
      conversion_status='Validation complete; free-weight promotion failed W1/W2. Existing fixed 2/3 K0 seasonal policy retained; all22 fits descriptive only.',
      expectations=expectations,street=street,
      ...
      horizon='13 September 2027: 256/365 of FY27 net cash/share flows from FY26 end, uniform within year. Full FY27 EBITDA. December 2027 separately shown.')
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--revenue-dir',type=Path,default=ROOT/'data/processed/forecast_methods/lane4_revenue_v2/snapshot_v1')
    ...
    out=ROOT/'data/processed/forecast_methods/lane4_model_v2'/args.run_id
    book=ROOT/'model/lane4_v2/outputs/lane4_model'/args.run_id
    ...
    for c in data['cases']:
        ...
        summary.append(dict(scenario=c['scenario'],scenario_label=c['label'],q4_revenue_musd=c['quarters'][3]['revenue'],q4_guide_musd=c['quarters'][3]['guide'],
            ...
            exit_multiple=data['globals']['exit_ev_ebitda'],**{k:v[k] for k in v if k!='lenses'}))
    ...
    dump(out/'horizon.json',{'information_date':str(INFO_DATE),'valuation_date':str(HORIZON),'days':256,'year_days':365,'fraction':HORIZON_FRACTION,'assumption':'Uniform FY27 net cash and net share flows from FY26 end; full FY27 EBITDA, no interpolation of EV multiple','later_comparison':str(FY27_END)})
    sources=[ROOT/'analysis/src/overnight/13_driver_model.py',ROOT/'model/ABNB_driver_model.xlsx',ROOT/'model/assumptions.md',
       ...
       ROOT/'data/processed/overnight/13_valuation_summary.csv',args.revenue_dir/'forecast.csv',args.revenue_dir/'operating_inputs.csv',args.revenue_dir/'expectations_comparison.csv',args.revenue_dir/'joint_scenarios.csv',ROOT/'data/processed/forecast_methods/lane4_sources_v2/snapshot_v1/integrity_receipt.json']
