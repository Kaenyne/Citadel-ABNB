...
```powershell
python -X utf8 -m pytest analysis/src/forecast_methods/cohort_fx_v1/tests -q
python -X utf8 analysis/src/forecast_methods/cohort_fx_v1/run.py --out data/processed/forecast_methods/cohort_fx_v1_new_run
```
...
Frozen canonical outputs are the files directly under `data/processed/forecast_methods/cohort_fx_v1/`. `qa_rebuild/` is an independent exact rerun with identical arguments. Numerical CSVs use USD, never implicit millions; notes may display USD millions.
