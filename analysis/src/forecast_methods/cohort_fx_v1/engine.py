...
def check_frame(frame, keys, numeric, as_of, meta=True):
    ...
    if meta:
        ...
        dates = pd.to_datetime(frame.information_date, errors="coerce", utc=True)
        if dates.isna().any() or (dates > pd.Timestamp(as_of, tz="UTC")).any():
...
class RateBook:
    def __init__(self, rates, as_of):
        ...
        for r in rates.itertuples():
            p = period(r.period)
            cutoff = pd.Timestamp(r.quote_cutoff)
            if pd.isna(cutoff) or cutoff > pd.Timestamp(as_of):
                raise ValueError("Future or missing quote cutoff")
            if r.rate_status == "observed" and p.end_time.normalize() > pd.Timestamp(as_of):
