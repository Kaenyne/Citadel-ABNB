...
def _regional(as_of, regional_gbv, regional_lambdas):
    ...
    l = _dated(regional_lambdas, as_of, ["region", "season"], ["lambda_pct"])
    l["season"] = pd.to_numeric(l.season, errors="raise").astype(int)
    if not l.season.isin([1, 2, 3, 4]).all():
        raise ValueError("Regional season must be an integer 1..4")
...
def pit_lambda(season, as_of, variant=None, *, panel=None, regional_gbv=None, regional_lambdas=None):
    ...
    if int(season) not in (1, 2, 3, 4):
        raise ValueError("season must be 1..4")
    p = _panel(as_of, panel)
    g, l = _regional(as_of, regional_gbv, regional_lambdas)
    if g is not None:
        ...
        return dict(season=int(season), regions=r.to_dict("records"), lambda_pct=None,
                    n_train=len(r), aggregation="sum regional dollar revenue, never lambda coefficients")
    rows = _lambda_rows(p)
...
def kernel_forecast(q, as_of, *, variant=None, panel=None, regional_gbv=None, regional_lambdas=None):
    ...
    result.pop("_draws")
    result.update(as_of=str(_date(as_of).date()), knowable_from=str(p.print_date.max().date()))
    return result


def _cushions(as_of, supplied=None):
    if supplied is None:
    ...
    f = f.sort_values("quarter").tail(8)
    if f.empty:
...
def kernel_guide(q, as_of, cushion="median", *, variant=None, panel=None,
    ...
    c = _cushions(as_of, cushion_frame)
    ...
    f.update(point=point, as_of=str(_date(as_of).date()), cushion=divisor-1,
             ...
             knowable_from=str(max(p.print_date.max(),c.print_date.max()).date()))
    calibration = _calibration(p, c, variant)
    ...
    f["conformal_caveat"] = "Chronological calibration; exchangeability not assumed; no coverage guarantee"
    return f


def _calibration(p, cushions, variant):
    ...
    for row in _lambda_rows(p).itertuples():
        ...
        pred = f["point"]/float(cr.ratio.median())
        errors.append(abs(pred-guide_actual)/pred)
    return dict(relative_errors=errors[-6:])
