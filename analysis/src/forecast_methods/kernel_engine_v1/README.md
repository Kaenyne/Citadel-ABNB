...
**STOP: Gate 1 failed on the initial acceptance test (1 failed, 46 passed). This module is unvalidated.** The reference-rounding boundary, full test output and outstanding checks are recorded in `docs/revenue-forecast-strategy/05_backtests/K0_KERNEL_ENGINE.md`. No default-selection results or accepted downstream outputs have been produced.

From the repository root, using its `.venv` interpreter:

```text
python -m pytest analysis/src/forecast_methods/kernel_engine_v1/tests -q
python analysis/src/forecast_methods/kernel_engine_v1/run.py --as-of 2026-09-12
```

New outputs only: `data/processed/forecast_methods/kernel_engine_v1/`. No registrations or frozen files are written.

## Public interface

Import `kernel_engine_v1.engine` with `analysis/src/forecast_methods` on `sys.path`. Currency outputs are **USD millions**; lambda is in **percent**.
...
The full-W1 LOO table is a **retrospective specification diagnostic**, not an out-of-sample alpha claim. The result and frozen live choice will be added after the registered run. The policy is frozen now.
