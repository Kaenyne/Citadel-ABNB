"""Rebuild K0 outputs: python analysis/src/forecast_methods/kernel_engine_v2/run.py."""
...
import engine as E
from acceptance import EXPECTED, within_reference_precision
...
def main():
    ...
    out=E.ROOT / "data/processed/forecast_methods/kernel_engine_v2"
    ...
    acceptance["match_reference_precision"]=[within_reference_precision(a,b) for a,b in zip(acceptance.lambda_pct,acceptance.expected_pct)]
    acceptance["display_2dp"]=acceptance.lambda_pct.map(lambda x:f"{x:.2f}")
    acceptance.to_csv(out/"acceptance.csv",index=False)
    specified=acceptance[acceptance.season.isin([3,4])]
    expected_display = specified.expected_pct.map(
        lambda x: float(Decimal(str(x)).quantize(Decimal(".01"), rounding=ROUND_HALF_UP))
    )
    ok = (len(acceptance) == 12
          and bool(acceptance.match_reference_precision.all())
          and bool((specified.lambda_pct.round(2) == expected_display).all()))
    print("acceptance test:","PASS on all 12 cells" if ok else "FAIL",flush=True)
    print(acceptance[["quarter","lambda_pct","expected_pct","display_2dp","match_reference_precision"]].to_string(index=False),flush=True)
    if not ok:
        return 2
