...
plt.rcParams.update({"font.family": "DejaVu Sans", "font.size": 11, "axes.titleweight": "bold", "axes.labelcolor": NAVY,
...
def main():
    ...
    ax = fig.add_axes([.055, .14, .89, .67]); ax.set_xlim(0, 1); ax.set_ylim(0, 1); ax.axis("off")
    ...
    box(ax, .715, .69, .27, .25, "Forecast we need", "Future GBV + conversion\n= expected revenue\n÷ (1 + cushion) = guide", NAVY)
    ...
    heading(fig, "Accuracy depends on the target and the sample", "Lower guide RMSE is better. These are reconstructed historical forecasts, not live performance.",
    ...
    ax.legend(loc="upper center", bbox_to_anchor=(.5, -.16), ncol=2, frameon=False, fontsize=10)
