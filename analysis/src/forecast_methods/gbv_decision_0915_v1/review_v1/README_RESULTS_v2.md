...
Full results, limitations, exact failed-attempt explanations and RESUME: `docs/revenue-forecast-strategy/05_backtests/GD_REVIEW_RESULTS_v2.md`. Both source tables and the frozen scorer use raw guide midpoints. The repo-required interval convention is handled by a separate additive sensitivity; the frozen scorer does not currently implement it. Preregistered new descriptive diagnostics are saved in `review_v1/horizon_v1/covariance_reconciliation.csv` and `regime_descriptive.csv`.
