"""fx_lag_v2 / fits.py -- COPY of fx_lag/fits.py.  Only change: object_a also returns
the 95% confidence-set grid points in its `pack`, so the 4Q26 interval can be built
by pushing every admissible weight vector through the forward baskets.

Stage (b) ADR contemporaneous fits, and Objects A / B / C.
...
def object_a(d: pd.DataFrame, target: str, label: str, grid_step=0.025, amax=0.90):
    ...
    return res, pd.DataFrame(vrows), (s, X, y, lo, hi, a_hat, f["sigma"], CS)
