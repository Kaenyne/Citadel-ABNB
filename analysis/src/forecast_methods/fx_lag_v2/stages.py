"""fx_lag_v2 / stages.py  -- COPY of fx_lag/stages.py.  Only the as-of date of the
forward schedule changes (2026-08-28 -> 2026-09-04, the last observation in the
refresh).  Everything else is byte-identical to the fx_lag original.

Hypothesis horse-race, bootstrap, determined share,
...
import pit_fx
from common import (OVN, OUT, METHOD, TODAY, FX_LAST_OBS, fit_interval_linear, interval_loglik,
...
def forward_schedule(a_hat, sigma, asof=None) -> pd.DataFrame:
    ...
    v2: asof defaults to FX_LAST_OBS = 2026-09-04, the last observation in the
    2026-09-11 refresh (FRED H.10 publishes with about a one-week lag)."""
    asof = asof or FX_LAST_OBS
    ...
    for pname, shift in paths.items():
        ...
        bk[pname] = bseries
    out = pd.DataFrame(rows)
    out["note"] = f"spot held constant from {asof} (last FRED obs in the 2026-09-11 refresh); +/-1sd = +/-5% parallel shift in the held spot"
