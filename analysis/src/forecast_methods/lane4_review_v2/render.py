...
def render(source: Path, output: Path):
    ...
    doc = SimpleDocTemplate(str(output), pagesize=(8.5 * inch, 11 * inch),
                            ...
                            title="Airbnb | Revenue, guide and value review", author="Citadel ABNB research - unsigned review")
    width = 7.30 * inch
    story = []
    paragraphs = []
    table_lines = []
    ...
    def page(canvas, doc):
        ...
        canvas.drawString(0.60 * inch, 0.23 * inch, "ABNB | Unsigned benchmark | Fixed K0 retained | Incremental FX ineligible")
    ...
    for i, p in enumerate(reader.pages, 1):
        ...
        if len(extracted) < 300 or "Fixed K0 retained | Incremental FX ineligible" not in extracted:
