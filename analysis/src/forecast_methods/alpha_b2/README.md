...
The historical consensus timestamp boundary was repaired after the independent vintage review. CONVENTION-authorized `YYYY-MM-DD` observations remain eligible. Explicit intraday timestamps require a timezone and must be strictly before 16:00 America/New_York on the letter day; exact close, later times and ambiguous naive intraday stamps are rejected. `exact_day` uses the New York date for explicit instants, and selection orders them by UTC. Date-only rows use local midnight solely as a deterministic sorting anchor, not a claimed source time. The input manifest now includes the kernel's actual quarterly KPI panel and guidance-cushion CSV dependencies.

Repair evidence: `data/processed/forecast_methods/alpha_b2/diagnostic_vintage_boundary_20260913T174852_833222Z/` contains original source/test snapshots, the initial failing regression receipt, 44 passing package tests, and a reproducible equivalence check. The isolated `--no-register` rebuild is `run_20260913T175201_016865Z/`: all seven research CSVs and the candidate are byte-identical to the metadata-corrected reference run; its FORMAT-expanded preview is byte-identical to the existing registry. No empirical output or registered forecast changed.
