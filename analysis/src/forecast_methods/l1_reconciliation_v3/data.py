"""l1-reconciliation-v3: copied v2 data loaders, with strict vintage exclusions.

R writes only to its new v3 output directory. Disclosed FX pairs are filtered
strictly before as_of; current estimated basket slopes are unavailable to
historical refits. Legacy denominator assumptions below are inherited for the
copied model interface; the R fit uses total units and total GBV, and does not
estimate or adopt a non-home composition scenario from these tables.
...
OUT = DATA / "forecast_methods" / "l1_reconciliation_v3"      # v2 writes HERE, only here
...
def fx_pp_table(intervals: pd.DataFrame, as_of=None) -> pd.DataFrame:
    ...
    # The supplied pass-through slopes have no historical parameter vintages.
    # Historical refits can therefore use disclosed letter pairs only. The current
    # basket is a full-sample/live decomposition input, never a PIT coefficient.
    if as_of is not None and as_of <= __import__("datetime").date(2026, 9, 11):
        result.loc[result.source != "disclosed_pair", "fx_pp"] = np.nan
    return result
