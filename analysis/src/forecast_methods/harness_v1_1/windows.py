...
def window_of_target(q: str):
    ...
    FORMAT 1.1: any quarter at or after the first LIVE target (2026Q3) that is not in the
    hard-coded list is LIVE as well, so 5 Nov scenarios for 2026Q4 / 1Q27 can be registered.
    Those rows still enter no metric and no gate (the scorer drops every LIVE row).
    """
    q = Q.canon(q)
    wins = [w for w, qs in WINDOW_MEMBERSHIP.items() if q in qs]
    if not wins and q >= LIVE_TARGETS[0]:
        wins = ["LIVE"]
    return wins
