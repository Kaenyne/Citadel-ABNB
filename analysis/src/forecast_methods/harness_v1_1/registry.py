"""Registry format v1.1 -- validator, writer and loader.

Identical to the frozen 1.0 validator except for ONE rule: a LIVE row may carry any
vintage_date in [LIVE_VINTAGE_MIN, RUN_DATE] (a real forecast date), not only a guide date
or the 1.0 constant 2026-09-11. W1 / W2 rows still require a guide date. See README.md.
"""
...
def validate_registry_frame(df: pd.DataFrame, *, strict_windows: bool = True) -> pd.DataFrame:
    ...
    # FORMAT 1.1: historical rows need a guide date; LIVE rows may carry a real forecast date.
    guide_dates = set(W.GUIDE_DATES_ALL)
    badv = []
    for r in d.itertuples():
        v = r.vintage_date
        if v is None:
            badv.append((r.window, None)); continue
        if v in guide_dates:
            continue
        if r.window == "LIVE" and P.LIVE_VINTAGE_MIN <= v <= P.RUN_DATE:
            continue
        badv.append((r.window, v))
    if badv:
        raise RegistryError(
            "vintage_date must be a guide date from calendar.csv (W1/W2/LIVE) or, for LIVE rows "
            f"only, a real forecast date in [{P.LIVE_VINTAGE_MIN}, RUN_DATE={P.RUN_DATE}]; "
            f"got {sorted(set(badv), key=str)[:6]}")
