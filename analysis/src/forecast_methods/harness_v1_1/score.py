#!/usr/bin/env python
"""Scorer (FORMAT 1.1): shared registry -> metric block -> scoreboard_v1_1.csv / .md.

Same arithmetic as the frozen 1.0 scorer; writes ONLY under data/processed/forecast_methods/harness_v1_1/.
Run:
  python analysis/src/forecast_methods/harness_v1_1/score.py
"""
...
if __package__ in (None, ""):
    ...
    from harness_v1_1 import quarters as Q
    from harness_v1_1 import metrics as M
    from harness_v1_1.registry import load_registry, QUANTILE_COLUMNS, QUANTILE_LEVELS
    from harness_v1_1.loaders import load_targets
else:
...
def _md(sb: pd.DataFrame) -> str:
    lines = ["# Harness scoreboard (FORMAT 1.1)", "",
             f"Format v{P.FORMAT_VERSION}. Built {P.RUN_DATE} (RUN_DATE). Same metrics as the frozen 1.0 "
             "scorer over the same shared registry; LIVE rows are excluded by construction and the "
             "2026-08-06 guide enters no metric. The frozen `harness/scoreboard.csv` remains the "
             "authoritative W1/W2 board; this file exists so LIVE rows dated after 2026-09-11 can be "
             "recorded and scored on 5 Nov.", "",
