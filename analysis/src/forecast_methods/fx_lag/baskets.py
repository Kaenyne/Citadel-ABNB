"""fx_lag / baskets.py -- Stage (a): the daily booking-date currency basket X^B.

Sources
-------
data/processed/overnight/10_fx_daily.csv    FRED bilateral dailies, USD per unit,
                                            9 currencies, 2018-01-02 .. 2026-08-28
...
FRED cache DTWEXBGS                         broad trade-weighted USD index
...
from common import OVN, L0, to_period, short, write
...
def broad_usd_quarterly() -> pd.Series:
    b = pd.read_csv(FRED_CACHE / "DTWEXBGS.csv", parse_dates=["observation_date"])
    b = b.dropna()
    b["q"] = b["observation_date"].dt.to_period("Q")
    avg = b.groupby("q")["DTWEXBGS"].mean()
...
def build() -> dict:
    ...
    last = pd.read_csv(OVN / "10_fx_daily.csv", parse_dates=["date"])
