...
DEFAULT_OUTPUT = ROOT / "data/processed/forecast_methods/lane4_revenue_v2/snapshot_v1"
...
def main():
    ...
    if args.as_of != "2026-09-13":
        raise SystemExit("This accepted-source integration is frozen at information date2026-09-13; a new vintage requires a new reviewed package")
    dated("2026-09-13", args.as_of)
    output.mkdir(parents=True)
    try:
        from integration import verify_accepted_source, integrate
        accepted = verify_accepted_source(args.as_of)
        ...
        for scenario, description in scenario_labels.items():
            ...
            for q in ["2026Q3", "2026Q4", "2027Q1"]:
                ...
                row.update(scenario=scenario, scenario_description=description,
                           ...
                           fx_integration_status="accepted_L3_research_financial_application_ineligible", incremental_fx_musd=None,
        ...
        # Accepted L3 source is fixed by immutable manifest. Research acceptance
        # does not satisfy the financial FX contract; no optional silent overlay.
        ...
        consensus["observation_status"] = consensus.as_of_timestamp.map(lambda x: "captured_at_frozen_information_snapshot_not_refreshed_on_execution" if str(x).startswith(args.as_of) else "older_dated_anchor_not_refreshed_on_execution")
        ...
        for r in forecasts:
            ...
            for c in consensus.to_dict("records"):
                ...
                comparisons.append(dict(scenario=r["scenario"], quarter=r["quarter"], guide_musd=r["guide_musd"],
                                        comparison_basis="different_objects_guide_minus_revenue_not_surprise",
        ...
        status = dict(as_of=args.as_of, l3_bundle_status="accepted_immutable_research", accepted_l3_inputs=1187,
                      conversion_status=CONVERSION_STATUS, conversion_integration_owner="L3 estimation and validation; no new weight fitted in L4",
                      l3_commit_content_verification="source worker verified108 manifest hashes104 lineage32 acceptance bindings; runner rechecks all108 hashes",
                      incremental_fx_musd=None, hedge_dollars=None, fx_neutral_revenue=None,
                      reason="Accepted research has unresolved baseline hedge split and scenario-only currency/RNPL exposures; no certified compatible pre-hedge multiplier",
        ...
        receipt = dict(as_of=args.as_of, completed_utc=datetime.now(timezone.utc).isoformat(), n_forecast_rows=len(forecasts), n_operating_rows=len(operating),
                       ...
                       missing_l3_inputs="certified compatible pre-hedge reference; baseline/new hedge dollars; identified surviving RNPL revenue exposure and rate fixing",
                       claimed_historical_validation="none; conditional review scenario only; W1=0 W2=0")
        extra = integrate(output, forecasts, operating, weights, consensus, guide_info, accepted, args.as_of)
