...
def apply_row(baseline, row, as_of):
    required = ["quarter", "metric", "scenario", "value", "lower", "upper", "units",
                ...
                "multiplier_accounting_basis", "scenario_hedge_musd", "scenario_hedge_source_ref", "scenario_hedge_evidence_status"]
    ...
    if row["multiplier_accounting_basis"] != "certified_matching_pre_hedge_revenue":
        raise ValueError("After-hedge or unresolved multiplier cannot be treated as pre-hedge")
    if row["hedge_treatment"] not in {"reconciled_baseline_hedges_held_unchanged", "explicit_reconciled_scenario_hedge"}:
    ...
    new_hedge = number(row["scenario_hedge_musd"], "scenario_hedge_musd")
    ...
    if not row["scenario_hedge_source_ref"] or row["scenario_hedge_evidence_status"] not in {"verified_scenario_reconciliation", "explicit_unchanged_hedge_assumption"}:
        raise ValueError("Missing explicit scenario hedge reconciliation or unchanged assumption")
    if row["hedge_treatment"] == "reconciled_baseline_hedges_held_unchanged" and (new_hedge != hedge or row["scenario_hedge_evidence_status"] != "explicit_unchanged_hedge_assumption"):
        raise ValueError("Unchanged-hedge assumption must explicitly equate H_new and H")
    ...
    adjusted = pre_hedge * multiplier + new_hedge
    number(adjusted, "adjusted consolidated revenue", True)
    result.update(scenario=row["scenario"], revenue_musd=adjusted,
                  ...
                  scenario_hedge_musd=new_hedge, incremental_pre_hedge_fx_musd=pre_hedge*(multiplier-1),
                  incremental_hedge_musd=new_hedge-hedge,
