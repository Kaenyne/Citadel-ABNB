...
from cohort_fx_v2.engine import apply_timing, from_reported_contributions, period, yoy_bridge
...
def rate_table(daily, periods, multiplier, as_of=AS_OF, source_date=SOURCE_DATE, max_stale_days=14):
    ...
    for c in ["USD", *FOREIGN]:
        ...
        if c!="USD":
            ...
            ref_dates=dc[dc.date.dt.year.eq(2025)].date
            if (len(ref_dates)<240 or (ref_dates.min()-pd.Timestamp("2025-01-01")).days>7
                    or (pd.Timestamp("2025-12-31")-ref_dates.max()).days>7
                    or ref_dates.diff().dt.days.dropna().max()>10):
                raise ValueError("Incomplete 2025 reference daily coverage")
        ...
        for p in sorted(periods):
            ...
            if c=="USD":
                ...
                quote_cutoff=min(pd.Timestamp(as_of),pd.Timestamp(source_date),end)
            else:
                obs=dc[dc.date.between(start,end)]
                observed_end=min(end,cutoff)
                observed_expected=len(pd.bdate_range(start,observed_end))
                if observed_expected and (len(obs)<.8*observed_expected
                        or obs.empty or (obs.date.min()-start).days>7
                        or (observed_end-obs.date.max()).days>7
                        or (len(obs)>1 and obs.date.diff().dt.days.dropna().max()>10)):
                    raise ValueError(f"Incomplete observed period coverage: {p}/{c}")
...
def source_manifest(bundle):
    refs=[(str(p.relative_to(ROOT)),p,"repo_read_only") for p in [KPI_PATH,FX_PATH,
        ...
        ROOT/"docs/revenue-forecast-strategy/05_backtests/L3_COHORT_FX_AUDIT_REPAIR_v2.md",
        ROOT/"docs/revenue-forecast-strategy/05_backtests/L3_COHORT_FX_PERIOD_COVERAGE_v2.md"]]
...
def run(out,bundle=None):
                ...
                for mult,rates in rate_sets.items():
                    ...
                    for metric,field,unit,treatment in [
                        ...
                        adapter.append(dict(quarter="2026Q3",metric=metric,scenario=name,value=s[field],
                            ...
                            evidence_status="scenario_only_unidentified_exposure_and_timing",source_ref="cohort_fx_v2/scenario_summary.csv; L3_COHORT_FX_ACCOUNTING.md",
    ...
    metadata=dict(information_date=AS_OF,package="cohort_fx_v2",research_status="PARTIAL",
