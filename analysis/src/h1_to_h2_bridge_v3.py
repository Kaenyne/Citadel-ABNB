"""H1 -> H2 bridge for ABNB, v3: nights and ex-FX ADR re-based to the team baseline and the ADR v3 card.

v3 CHANGES (12 Sep 2026, Krish; v2 is otherwise carried verbatim, v1 and v2 outputs untouched):
  * nights_yoy_pct 3Q26/4Q26: the v1/v2 pattern + unfitted overlays (RNPL lap -1.5/-2.5, World Cup +0.5) are
    replaced by the team baseline: 3Q26 +9.9% (146.8mm, PR #32 base = reviews-index read; band 8.5 to 11.0)
    and 4Q26 +8.1% (131.8mm, ADR v3 N memo case B, the adopted baseline; case A +8.9% / 132.7mm is the top
    of the band). Sources: data/processed/nights_baseline_reconciliation.csv and adrv3/P/adr_card_v3.csv.
    The old overlays are kept as comparison rows with pts = 0 applied.
  * adr_yoy_exfx_pct 3Q26/4Q26: the v1/v2 pattern + seats-dilution overlay is replaced by the ADR v3 card's
    ex-FX line (variant set by ADOPT_VARIANT below; the other variant is a comparison column). The card
    already carries new-business dilution (-0.48) inside its ex-FX, so the v1 seats overlay is retired
    (pts = 0) to avoid double counting.
  * The FX lines stay as in v2 (ADR v3 midpoint; fx_lag_v2 kernel +1.0 for 4Q26 revenue FX).
  * Consistency check printed and written: the bridge's reported ADR and 3Q26 GBV must reproduce the card's
    (adr_usd_point, gbv_busd_point) to rounding, since they now share every input.
  * Nights scenarios re-cut to the baseline band (3Q26 8.5 / 9.9 / 11.0; 4Q26 8.0 / 8.1 / 8.9), with the
    v1/v2 lap scenarios kept as comparison rows.
  * New outputs: h2_bridge_v3_rebased_lines.csv, h2_bridge_v3_vs_v2_delta.csv, h2_bridge_v3_card_check.csv.
  * Output directory data/processed/h2_bridge_v3/. Nothing registered through the harness.

v2 docstring follows.

H1 -> H2 bridge for ABNB, v2: the FX lines swapped to the programme's adopted estimators.

v2 CHANGES (12 Sep 2026, Krish; everything else is the 10 Sep v1 script verbatim):
  * ADR FX (fx_pts_adr) for 3Q26/4Q26 now uses the ADR v3 midpoint estimator (mean of the EUR fit and
    the regional-basket build; data/processed/adrv3/N/N1_fx_choice_card.csv; RMSE 0.33pp on 1Q24-2Q26)
    instead of v1's single-factor broad-USD fit (0.5 - 0.72 x USD y/y). The broad-USD fit is kept as a
    comparison column.
  * Revenue FX (fx_pts_revenue) for 4Q26 now uses the kernel-implied output of fx_lag_v2 (Phi kernel
    (0, 2/3, 1/3) x 0.851 on the revenue-weighted basket, spot held; B4_FX_EXHIBIT.md) instead of v1's
    assumed 2.0 (range 1-3). 3Q26 stays at management's stated ~3 (kernel says +2.9). The EUR-only
    05_fx_schedule fit, the guide-anchored carry and the v1 assumption are kept as comparison columns.
  * New outputs: h2_bridge_v2_fx_line.csv (adopted vs comparison values, sources, data-through date)
    and h2_bridge_v2_vs_v1_delta.csv (what the swap moved in the projection and revenue dollars).
  * Output directory is data/processed/h2_bridge_v2/ (v1 outputs are never overwritten).
  * Nothing here is registered through the harness: the FX objects are already registered by
    fx-lag-v2 (fx-lag-v2__fx_pts_revenue_2026q4_v2). No FX pp is added on top of the lagged-GBV
    revenue-dollar path; the ADR FX enters GBV and is carried through the lag, which is the kernel.
  * Not changed and out of scope: the nights overlays (RNPL lap -1.5/-2.5, World Cup +0.5), which the
    10 Sep reconciliation says should be re-based to the team baseline; see docs/RNPL_HANDOFF.md.
...
OUT = os.path.join(ROOT, 'data', 'processed', 'h2_bridge_v3')
V2 = os.path.join(ROOT, 'data', 'processed', 'h2_bridge_v2')  # v2 outputs, read-only, for the delta table
ADOPT_VARIANT = 'v3_with_K'   # ADR v3 card variant carried as the point; the other is a comparison column
V1 = os.path.join(ROOT, 'data', 'processed', 'h2_bridge')  # v1 outputs, read-only, for the delta table
FXV2 = os.path.join(ROOT, 'data', 'processed', 'forecast_methods', 'fx_lag_v2')
N1 = os.path.join(ROOT, 'data', 'processed', 'adrv3', 'N')
...
events = [
 ...
 ('3Q26', 'FX tailwind fades: ADR FX from the ADR v3 midpoint estimator; revenue FX from the fx_lag_v2 kernel (booking-date FX carried through the 2/3-1/3 GBV lag): ~3 pts Q3, ~1 pt Q4', '2026-07..12', 'spot, continuous', 'FX pts ADR/revenue', 'down (less tailwind)', 'see h2_bridge_v2_fx_line.csv', 'yes (spot FX; hedging program)', 'yes (Q3 revenue ~3 pts)', 'adrv3/N1 card; fx_lag_v2 23_forecast_4q26_v2 (B4_FX_EXHIBIT.md)'),
...
for lab, (s, e) in QTRS.items():
    ...
    fxrows.append(dict(quarter=lab, usd_broad_avg=u, usd_broad_yoy_pct=usd_yoy, eurusd_avg=eu, eurusd_yoy_pct=eur_yoy,
                       obs_days=n_obs, filled_days_flat=n_fill, fx_pts_adr_usd_broad_fit_v1=0.5 - 0.72 * usd_yoy,
                       fx_pts_adr_disclosed=pq.fx_pts_adr.get(lab, np.nan)))
fx = pd.DataFrame(fxrows)

# v2: ADR FX from the ADR v3 midpoint estimator (N1 card for 3Q26/4Q26; N1 by-quarter file for the 1H26 reconciliation)
n1_card = pd.read_csv(os.path.join(N1, 'N1_fx_choice_card.csv'))
n1_hist = pd.read_csv(os.path.join(N1, 'N1_fx_estimator_by_quarter.csv')).set_index('quarter_h')
n1_meta = json.loads(open(os.path.join(ROOT, 'data/processed/overnight2/B/B_fx_meta.json')).read())
n1_fx_through = pd.Timestamp(n1_meta['latest_fred_fx_observation'])
for est in ('eur_fit', 'baskets', 'midpoint'):
    col = {'eur_fit': 'est_from_eur', 'baskets': 'est_from_regional_baskets', 'midpoint': 'est_from_midpoint'}[est]
    vals = {}
    for lab in QTRS:
        if lab in n1_hist.index:
            vals[lab] = float(n1_hist.loc[lab, col])
        else:
            r = n1_card[(n1_card.quarter == lab) & (n1_card.fx_estimator == est)]
            vals[lab] = float(r.fx_effect_pp.iloc[0]) if len(r) else np.nan
    fx[f'fx_pts_adr_{est}'] = fx.quarter.map(vals)
fx['fx_pts_adr_adopted'] = fx['fx_pts_adr_midpoint']
fx['fx_pts_adr_adopted_source'] = 'ADR v3 midpoint of EUR fit and regional baskets (adrv3/N/N1_fx_choice_card.csv)'
fx['fx_data_through'] = n1_fx_through.date()
fx.to_csv(os.path.join(OUT, 'h2_bridge_fx.csv'), index=False)
fxq = fx.set_index('quarter')

# v2: revenue FX from the fx_lag_v2 kernel output (spot held), with the rejected constructions as comparison columns
k4 = pd.read_csv(os.path.join(FXV2, '23_forecast_4q26_v2.csv'))
k4 = k4[k4.path == 'spot_held'].set_index('quarter')
kernel_fx_through = pd.Timestamp(k4.fx_data_through.iloc[0])
fourway = pd.read_csv(os.path.join(FXV2, '24_four_way_4q26.csv'))
fx_sched = pd.read_csv(os.path.join(ROOT, 'data/processed/overnight/05_fx_schedule.csv'))
fx_sched = fx_sched[fx_sched.path == 'consensus'].set_index('quarter')
rev_fx_kernel = {q: float(k4.loc[q, 'point_phi_basket_scale_0.851_pp']) for q in ('3Q26', '4Q26')}
rev_fx_kernel_cs = {q: (float(k4.loc[q, 'cs_interval_lo_pp']), float(k4.loc[q, 'cs_interval_hi_pp'])) for q in rev_fx_kernel}
rev_fx_kernel_b80 = {q: (float(k4.loc[q, 'band80_lo_pp']), float(k4.loc[q, 'band80_hi_pp'])) for q in rev_fx_kernel}
rev_fx_kernel_alt = {q: float(k4.loc[q, 'point_phi_basket_fitted_scale_pp']) for q in rev_fx_kernel}   # Phi x 0.653
rev_fx_free = {q: float(k4.loc[q, 'point_free_fit_stated_pp']) for q in rev_fx_kernel}
rev_fx_eur_only = {'3Q26': float(fx_sched.loc['2026Q3', 'revenue_fx_fit_pp']), '4Q26': float(fx_sched.loc['2026Q4', 'revenue_fx_fit_pp'])}
rev_fx_guide_anchored = {'3Q26': 3.0, '4Q26': float(fourway[fourway.construction.str.startswith('guide-anchored')].fx_level_pp.iloc[0])}
rev_fx_v1 = {'3Q26': 3.0, '4Q26': 2.0}
# the same Phi lag pushed through the ADR v3 midpoint ADR-FX points (the FX the revenue-dollar path below implicitly carries via GBV)
rev_fx_phi_on_adr_midpoint = {'3Q26': (2 / 3) * pq.loc['2Q26', 'fx_pts_adr'] + (1 / 3) * pq.loc['1Q26', 'fx_pts_adr'],
                              '4Q26': (2 / 3) * fxq.loc['3Q26', 'fx_pts_adr_adopted'] + (1 / 3) * pq.loc['2Q26', 'fx_pts_adr']}
REV_FX_ADOPTED = {'3Q26': 3.0, '4Q26': rev_fx_kernel['4Q26']}   # 3Q26: management's stated ~3 after hedging (kernel +2.9 agrees)
if not (last.date() == n1_fx_through.date() == kernel_fx_through.date()):
    print(f'WARNING: FX data-through dates differ: FRED raw {last.date()}, ADR v3 {n1_fx_through.date()}, fx_lag_v2 {kernel_fx_through.date()}')
...
# v3: team nights baseline and the ADR v3 card
nb = pd.read_csv(os.path.join(ROOT, 'data/processed/nights_baseline_reconciliation.csv'))
nb_base = nb[nb.model.str.startswith('PR #32 NA-lap model, base')].iloc[0]
card = pd.read_csv(os.path.join(ROOT, 'data/processed/adrv3/P/adr_card_v3.csv'))
card = card[card.fx_estimator == 'midpoint']
c3 = card[(card.quarter == '3Q26') & (card.variant == ADOPT_VARIANT)].iloc[0]
c4 = card[(card.quarter == '4Q26') & (card.variant == ADOPT_VARIANT) & (card.nights_case == 'N_case_B_global_lap_baseline')].iloc[0]
c4a = card[(card.quarter == '4Q26') & (card.variant == ADOPT_VARIANT) & (card.nights_case == 'N_case_A_team_baseline_top_of_band')].iloc[0]
other_variant = 'v3_without_K' if ADOPT_VARIANT == 'v3_with_K' else 'v3_with_K'
o3 = card[(card.quarter == '3Q26') & (card.variant == other_variant)].iloc[0]
o4 = card[(card.quarter == '4Q26') & (card.variant == other_variant) & (card.nights_case == 'N_case_B_global_lap_baseline')].iloc[0]
NIGHTS = {'3Q26': dict(point=float(nb_base.q3_yoy_pct), mm=float(nb_base.q3_nights_mm), lo=8.5, hi=11.0,
                       source='team baseline (PR #32 base, nights_baseline_reconciliation.csv); band = reviews stays index 8.5 to 11.0 (q3nowcast E)'),
          '4Q26': dict(point=float(c4.nights_yoy_pp), mm=float(c4.nights_m), lo=8.0, hi=float(c4a.nights_yoy_pp),
                       source='ADR v3 N memo case B (WS-D global lap, 45% ex-NA split), adopted baseline; lo = case B range 8.0; hi = case A 8.9 (PR #32 NA-only lap)')}
EXFX = {'3Q26': dict(point=float(c3.adr_exfx_yoy_pp), lo=float(c3.adr_exfx_central_lo_pp), hi=float(c3.adr_exfx_central_hi_pp), other=float(o3.adr_exfx_yoy_pp)),
        '4Q26': dict(point=float(c4.adr_exfx_yoy_pp), lo=float(c4.adr_exfx_central_lo_pp), hi=float(c4.adr_exfx_central_hi_pp), other=float(o4.adr_exfx_yoy_pp))}
assert abs(float(c3.fx_effect_pp) - fxq.loc['3Q26', 'fx_pts_adr_adopted']) < 1e-6 and abs(float(c4.fx_effect_pp) - fxq.loc['4Q26', 'fx_pts_adr_adopted']) < 1e-6, 'card FX != bridge FX'
fx3, fx4 = fxq.loc['3Q26', 'fx_pts_adr_adopted'], fxq.loc['4Q26', 'fx_pts_adr_adopted']
fx3_v1, fx4_v1 = fxq.loc['3Q26', 'fx_pts_adr_usd_broad_fit_v1'], fxq.loc['4Q26', 'fx_pts_adr_usd_broad_fit_v1']
rev_fx4_v1_formula = (2 / 3) * fx3_v1 + (1 / 3) * pq.loc['2Q26', 'fx_pts_adr']  # v1's own (unused) lag rule, kept for the comparison table

overlays = [
    ('nights_yoy_pct', '3Q26', f'v3: replace pattern with the team baseline {NIGHTS["3Q26"]["point"]:.2f}% ({NIGHTS["3Q26"]["mm"]:.1f}mm); {NIGHTS["3Q26"]["source"]}', NIGHTS['3Q26']['point'] - proj.loc['nights_yoy_pct', 'q3_pattern']),
    ('nights_yoy_pct', '4Q26', f'v3: replace pattern with the team baseline {NIGHTS["4Q26"]["point"]:.2f}% ({NIGHTS["4Q26"]["mm"]:.1f}mm); {NIGHTS["4Q26"]["source"]}', NIGHTS['4Q26']['point'] - proj.loc['nights_yoy_pct', 'q4_pattern']),
    ('nights_yoy_pct', '3Q26', 'RETIRED in v3 (comparison only; v1/v2 applied -1.5): RNPL US lap, half assumed to lap in Q3 -- unfitted, and the baseline already carries the lap', 0.0),
    ('nights_yoy_pct', '4Q26', 'RETIRED in v3 (comparison only; v1/v2 applied -2.5): RNPL US lap, fuller -- unfitted; the case B global lap in the baseline replaces it', 0.0),
    ('nights_yoy_pct', '3Q26', 'RETIRED in v3 (comparison only; v1/v2 applied +0.5): World Cup residual -- WS10 says no lift to 3Q26 booked nights', 0.0),
    ('nights_yoy_pct', '3Q26', 'Middle East cancellation drag (was -1 in 1Q26, not cited in 2Q26): assumed faded', 0.0),
    ('fx_pts_adr', '3Q26', f'replace pattern with the ADR v3 midpoint FX estimator (mean of EUR fit and regional baskets; FX through {n1_fx_through.date()}, flat spot after)', fx3 - proj.loc['fx_pts_adr', 'q3_pattern']),
    ('fx_pts_adr', '4Q26', f'replace pattern with the ADR v3 midpoint FX estimator (mean of EUR fit and regional baskets; FX through {n1_fx_through.date()}, flat spot after)', fx4 - proj.loc['fx_pts_adr', 'q4_pattern']),
    ('adr_yoy_exfx_pct', '3Q26', f'v3: replace pattern with the ADR v3 card ex-FX ({ADOPT_VARIANT}, midpoint FX) {EXFX["3Q26"]["point"]:+.2f}, central {EXFX["3Q26"]["lo"]:+.2f} to {EXFX["3Q26"]["hi"]:+.2f}; {other_variant} {EXFX["3Q26"]["other"]:+.2f}', EXFX['3Q26']['point'] - proj.loc['adr_yoy_exfx_pct', 'q3_pattern']),
    ('adr_yoy_exfx_pct', '4Q26', f'v3: replace pattern with the ADR v3 card ex-FX ({ADOPT_VARIANT}, midpoint FX) {EXFX["4Q26"]["point"]:+.2f}, central {EXFX["4Q26"]["lo"]:+.2f} to {EXFX["4Q26"]["hi"]:+.2f}; {other_variant} {EXFX["4Q26"]["other"]:+.2f}', EXFX['4Q26']['point'] - proj.loc['adr_yoy_exfx_pct', 'q4_pattern']),
    ('adr_yoy_exfx_pct', '3Q26', f'RETIRED in v3 (comparison only; v1/v2 applied {float(seats_mid.get("3Q26", -0.5)):+.2f}): seats/hotel dilution -- already inside the card ex-FX as new_business_seats -0.48', 0.0),
    ('adr_yoy_exfx_pct', '4Q26', f'RETIRED in v3 (comparison only; v1/v2 applied {float(seats_mid.get("4Q26", -0.5)):+.2f}): seats/hotel dilution -- already inside the card ex-FX as new_business_seats -0.48', 0.0),
    ('fx_pts_revenue', '3Q26', f'management: ~3 pts FX in the Q3 revenue guide (after hedging); fx_lag_v2 kernel gives {rev_fx_kernel["3Q26"]:+.1f} on the same basis', REV_FX_ADOPTED['3Q26'] - proj.loc['fx_pts_revenue', 'q3_pattern']),
    ('fx_pts_revenue', '4Q26', f'fx_lag_v2 kernel output: Phi (0, 2/3, 1/3) x 0.851 on the revenue-weighted basket, spot held from {kernel_fx_through.date()}; CS {rev_fx_kernel_cs["4Q26"][0]:+.1f} to {rev_fx_kernel_cs["4Q26"][1]:+.1f}; replaces v1 assumed 2.0 (range 1-3)', REV_FX_ADOPTED['4Q26'] - proj.loc['fx_pts_revenue', 'q4_pattern']),
]
...
scen = []
for name, n3, n4 in (('v3 baseline band LOW (3Q26 reviews-index low; 4Q26 case B low)', NIGHTS['3Q26']['lo'], NIGHTS['4Q26']['lo']),
                     ('v3 BASELINE (3Q26 team baseline; 4Q26 case B global lap)', NIGHTS['3Q26']['point'], NIGHTS['4Q26']['point']),
                     ('v3 baseline band HIGH (3Q26 reviews-index high; 4Q26 case A NA-only lap)', NIGHTS['3Q26']['hi'], NIGHTS['4Q26']['hi'])):
    scen.append(dict(scenario=name, rnpl_lap_q3_pts=np.nan, rnpl_lap_q4_pts=np.nan, nights_q3=n3, nights_q4=n4,
...
for name, lap3, lap4 in (('COMPARISON v1/v2: no lap (international RNPL adoption offsets the US anniversary)', 0.0, 0.0),
                         ('COMPARISON v1/v2: half lap', -1.5, -2.5), ('COMPARISON v1/v2: full lap (all ~3 pts of 1Q26 RNPL lift laps)', -3.0, -3.0)):
...
# ---- v2: the FX line table (adopted vs comparisons) and the v1 -> v2 delta
fxline = []
for q in ('3Q26', '4Q26'):
    fxline.append(dict(quarter=q, line='fx_pts_adr', adopted_pp=fxq.loc[q, 'fx_pts_adr_adopted'],
                       adopted_source='ADR v3 midpoint (N1 card): mean of EUR fit and regional baskets; RMSE 0.33pp on 1Q24-2Q26 vs 0.46 EUR / 0.42 baskets',
                       v1_value_pp=fxq.loc[q, 'fx_pts_adr_usd_broad_fit_v1'], v1_source='broad-USD single-factor fit 0.5 - 0.72 x USD y/y (predictive note 03)',
                       cmp_eur_fit_pp=fxq.loc[q, 'fx_pts_adr_eur_fit'], cmp_baskets_pp=fxq.loc[q, 'fx_pts_adr_baskets'],
                       cmp_usd_broad_fit_pp=fxq.loc[q, 'fx_pts_adr_usd_broad_fit_v1'],
                       cmp_eur_only_schedule_pp=np.nan, cmp_guide_anchored_pp=np.nan, cmp_free_fit_pp=np.nan, cmp_kernel_alt_scale_0653_pp=np.nan, cmp_phi_on_adr_v3_midpoint_pp=np.nan,
                       cs_lo_pp=np.nan, cs_hi_pp=np.nan, band80_lo_pp=np.nan, band80_hi_pp=np.nan,
                       management=np.nan, fx_data_through=n1_fx_through.date(),
                       note='enters GBV y/y through reported ADR and is carried to revenue through the 2/3-1/3 GBV lag; never added to revenue separately'))
    fxline.append(dict(quarter=q, line='fx_pts_revenue', adopted_pp=REV_FX_ADOPTED[q],
                       adopted_source=('management stated ~3 after hedging (6 Aug letter); kernel agrees at +2.9' if q == '3Q26'
                                       else 'fx_lag_v2 kernel-implied: Phi (0, 2/3, 1/3) x 0.851 on the revenue-weighted basket, spot held (B4_FX_EXHIBIT.md; registry fx-lag-v2__fx_pts_revenue_2026q4_v2)'),
                       v1_value_pp=rev_fx_v1[q], v1_source=('management ~3' if q == '3Q26' else 'assumed 2.0 (range 1-3)'),
                       cmp_eur_fit_pp=np.nan, cmp_baskets_pp=np.nan, cmp_usd_broad_fit_pp=np.nan,
                       cmp_eur_only_schedule_pp=rev_fx_eur_only[q], cmp_guide_anchored_pp=rev_fx_guide_anchored[q],
                       cmp_free_fit_pp=rev_fx_free[q], cmp_kernel_alt_scale_0653_pp=rev_fx_kernel_alt[q], cmp_phi_on_adr_v3_midpoint_pp=rev_fx_phi_on_adr_midpoint[q],
                       cs_lo_pp=rev_fx_kernel_cs[q][0], cs_hi_pp=rev_fx_kernel_cs[q][1], band80_lo_pp=rev_fx_kernel_b80[q][0], band80_hi_pp=rev_fx_kernel_b80[q][1],
                       management=(3.0 if q == '3Q26' else np.nan), fx_data_through=kernel_fx_through.date(),
                       note=(f'kernel point for 3Q26 is {rev_fx_kernel["3Q26"]:+.2f}; the letter-stated pp is already after hedges, so the 28_fx_hedge_forward line is never added on top' if q == '3Q26'
                             else f'v1 formula (2/3 x ADR-FX 3Q26 + 1/3 x ADR-FX 2Q26 on the broad-USD fit) would have given {rev_fx4_v1_formula:+.2f}; the EUR-only 05_fx_schedule fit and the guide-anchored carry are REJECTED constructions kept for comparison (B4 section 5)')))
fxline = pd.DataFrame(fxline)
fxline.to_csv(os.path.join(OUT, 'h2_bridge_v3_fx_line.csv'), index=False)

v1_proj = pd.read_csv(os.path.join(V1, 'h2_bridge_2026_projection.csv')).set_index('metric')
v1_rev = pd.read_csv(os.path.join(V1, 'h2_bridge_revenue_dollars.csv')).set_index('quarter')
rt = revtab.set_index('quarter')
delta = []
for m in ('fx_pts_adr', 'adr_yoy_reported_pct', 'gbv_yoy_reported_pct', 'fx_pts_revenue', 'revenue_yoy_reported_pct'):
    for q in ('q3', 'q4'):
        delta.append(dict(object=f'{m} {q}_adjusted', unit='pct or pts', v1=v1_proj.loc[m, f'{q}_adjusted'], v2=proj.loc[m, f'{q}_adjusted'],
                          delta=proj.loc[m, f'{q}_adjusted'] - v1_proj.loc[m, f'{q}_adjusted']))
for q in ('3Q26', '4Q26'):
    for c in ('lagged_gbv_busd', 'revenue_musd', 'revenue_low', 'revenue_high', 'yoy_pct', 'implied_guide_mid_if_cushion_holds', 'gbv_3q26_assumed_busd'):
        v1v, v2v = v1_rev.loc[q, c], rt.loc[q, c]
        delta.append(dict(object=f'{c} {q}', unit='$bn / $m / pct', v1=v1v, v2=v2v, delta=v2v - v1v))
delta = pd.DataFrame(delta)
delta.to_csv(os.path.join(OUT, 'h2_bridge_v3_vs_v1_delta.csv'), index=False)

# ---- v3: the re-based lines, the card consistency check, and the v2 -> v3 delta
rebased = []
for q, qq in (('3Q26', 'q3'), ('4Q26', 'q4')):
    v2p = pd.read_csv(os.path.join(V2, 'h2_bridge_2026_projection.csv')).set_index('metric')
    rebased.append(dict(quarter=q, line='nights_yoy_pct', adopted=NIGHTS[q]['point'], adopted_mm=NIGHTS[q]['mm'], band_lo=NIGHTS[q]['lo'], band_hi=NIGHTS[q]['hi'],
                        adopted_source=NIGHTS[q]['source'], v2_value=v2p.loc['nights_yoy_pct', f'{qq}_adjusted'],
                        v2_source='pattern (H1 mean + 2023-25 transition) + unfitted overlays (RNPL lap -1.5/-2.5, World Cup +0.5)',
                        pattern_only=proj.loc['nights_yoy_pct', f'{qq}_pattern'], comparison_other_variant=np.nan))
    rebased.append(dict(quarter=q, line='adr_yoy_exfx_pct', adopted=EXFX[q]['point'], adopted_mm=np.nan, band_lo=EXFX[q]['lo'], band_hi=EXFX[q]['hi'],
                        adopted_source=f'ADR v3 card ({ADOPT_VARIANT}, midpoint FX): last_q residual + measured mix + assumed fills' + (' + K mechanics line' if ADOPT_VARIANT == 'v3_with_K' else ''),
                        v2_value=v2p.loc['adr_yoy_exfx_pct', f'{qq}_adjusted'], v2_source='pattern (H1 mean + transition) + seats dilution overlay',
                        pattern_only=proj.loc['adr_yoy_exfx_pct', f'{qq}_pattern'], comparison_other_variant=EXFX[q]['other']))
rebased = pd.DataFrame(rebased)
rebased.to_csv(os.path.join(OUT, 'h2_bridge_v3_rebased_lines.csv'), index=False)

check = pd.DataFrame([
    dict(quarter='3Q26', object='ADR reported y/y, %', bridge=proj.loc['adr_yoy_reported_pct', 'q3_adjusted'], card=float(c3.adr_reported_yoy_pp)),
    dict(quarter='3Q26', object='ADR $', bridge=pq.adr_usd['3Q25'] * (1 + proj.loc['adr_yoy_reported_pct', 'q3_adjusted'] / 100), card=float(c3.adr_usd_point)),
    dict(quarter='3Q26', object='nights, mm', bridge=pq.nights_m['3Q25'] * (1 + proj.loc['nights_yoy_pct', 'q3_adjusted'] / 100), card=float(c3.nights_m)),
    dict(quarter='3Q26', object='GBV, $bn', bridge=gbv3, card=float(c3.gbv_busd_point)),
    dict(quarter='4Q26', object='ADR reported y/y, %', bridge=proj.loc['adr_yoy_reported_pct', 'q4_adjusted'], card=float(c4.adr_reported_yoy_pp)),
    dict(quarter='4Q26', object='ADR $', bridge=pq.adr_usd['4Q25'] * (1 + proj.loc['adr_yoy_reported_pct', 'q4_adjusted'] / 100), card=float(c4.adr_usd_point)),
    dict(quarter='4Q26', object='nights, mm', bridge=pq.nights_m['4Q25'] * (1 + proj.loc['nights_yoy_pct', 'q4_adjusted'] / 100), card=float(c4.nights_m)),
    dict(quarter='4Q26', object='GBV, $bn', bridge=pq.gbv_busd['4Q25'] * (1 + proj.loc['gbv_yoy_reported_pct', 'q4_adjusted'] / 100), card=float(c4.gbv_busd_point)),
])
check['diff'] = check.bridge - check.card
check['note'] = 'card GBV uses its own ADR $ x nights; bridge GBV compounds y/y rates on the 2025 base -- agreement to rounding is the test'
check.to_csv(os.path.join(OUT, 'h2_bridge_v3_card_check.csv'), index=False)

v2_proj = pd.read_csv(os.path.join(V2, 'h2_bridge_2026_projection.csv')).set_index('metric')
v2_rev = pd.read_csv(os.path.join(V2, 'h2_bridge_revenue_dollars.csv')).set_index('quarter')
rt3 = revtab.set_index('quarter')
d23 = []
for m in ('nights_yoy_pct', 'adr_yoy_exfx_pct', 'fx_pts_adr', 'adr_yoy_reported_pct', 'gbv_yoy_reported_pct', 'fx_pts_revenue', 'revenue_yoy_reported_pct'):
    for q in ('q3', 'q4'):
        d23.append(dict(object=f'{m} {q}_adjusted', unit='pct or pts', v2=v2_proj.loc[m, f'{q}_adjusted'], v3=proj.loc[m, f'{q}_adjusted'], delta=proj.loc[m, f'{q}_adjusted'] - v2_proj.loc[m, f'{q}_adjusted']))
for q in ('3Q26', '4Q26'):
    for c in ('lagged_gbv_busd', 'revenue_musd', 'revenue_low', 'revenue_high', 'yoy_pct', 'implied_guide_mid_if_cushion_holds', 'gbv_3q26_assumed_busd'):
        d23.append(dict(object=f'{c} {q}', unit='$bn / $m / pct', v2=v2_rev.loc[q, c], v3=rt3.loc[q, c], delta=rt3.loc[q, c] - v2_rev.loc[q, c]))
d23 = pd.DataFrame(d23)
d23.to_csv(os.path.join(OUT, 'h2_bridge_v3_vs_v2_delta.csv'), index=False)
...
print('\n=== v2 FX LINE')
print(fxline.to_string(index=False))
print('\n=== v2 vs v1 DELTA')
print(delta.to_string(index=False))
print('\n=== v3 REBASED LINES')
print(rebased.to_string(index=False))
print('\n=== v3 CARD CHECK')
print(check.to_string(index=False))
print('\n=== v3 vs v2 DELTA')
print(d23.to_string(index=False))
