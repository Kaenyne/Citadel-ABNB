...
| Diluted shares (start) | 597M | same | same | 2Q26 **diluted weighted-average** shares (XBRL), used as a **proxy** for the 30 Jun 2026 period-end fully diluted count. See "Model conventions" section 2 below |
...
| Regulatory nights drag, incremental pp of growth (NA / EMEA), FY26 / FY27 / FY28 | 1.67x the WS11 median | **0.03/0.36, 0.05/0.71, 0.07/0.96 pp (1.67x the WS11 median)** | 0 | WS11 `11_regulatory_overlay.csv`, regenerated after the WS22 (audit A06) conditional-dependency repair: cumulative EMEA median drag at 2028 fell 2.07% -> 2.03%, so the FY2028 incremental drag is 0.96pp (was 1.00pp; changed 7 Sep 2026, WS26). WS11 asks for the mean in the base and mean/median = 1.67. WS01 asks for it on EMEA nights, not on global revenue | NEW |
...
## Model conventions, decided 7 Sep 2026 (workstream 25, audit finding A12)

*These are conventions, not new numbers. Nothing below changes a value anywhere in the model; they say
what the existing values mean so a reviewer stops having to guess.*

### 1. The valuation outputs are 12-month forward targets, not present fair values

**Decision.** Every price on the `Valuation` sheet, in `13_valuation_summary.csv` and in the football
field is a **12-month target**, with an adopted **target date of ~30 September 2027** and **FY2027E exit
metrics**. The "upside" column is therefore a **12-month expected price return** from the 4 September 2026
close of $181.94 - *not* a discount to a present discounted value. Do not write "fair value", "worth
today" or "intrinsic value" next to any of these numbers; write "12-month target".

Why this and not a present value: `valuation()` in `13_driver_model.py` seeds the DCF with **FY2027 FCF**,
discounts the next annual flow one period, and adds **FY2027 net cash** divided by **FY2027 period-end
shares**. That is a forward convention in every line. Converting it to a present value would mean
discounting the target back and adding the FY2026-27 interim flows explicitly, which nobody has done.

Lens-by-lens conventions, including which year's metric, net cash and share count each lens uses, are in
**`data/processed/overnight/25_valuation_conventions.csv`**. Three things a reader must know:

- **All six football-field lenses now sit at the same ~30 Sep 2027 target date (revised 7 Sep 2026,
  workstream 26).** WS25 found that five lenses valued the company at end-FY2027 while the sixth
  (EV / adj. EBITDA on **FY2028E**) valued it at end-FY2028 and was averaged in undiscounted, lifting
  the base football-field mean by **$11.46** ($160.22 with it, $148.76 without). **Decision: discount
  the FY2028E lens one year at the scenario's cost of equity** (10.5% base, `Inputs` "Cost of equity"),
  so every lens in the field is a value at the adopted target date. Base FY2028E lens **$217.59 →
  $196.92**; base football-field mean **$160.22 → $156.79**; base football-field high $217.51 → $196.92.
  The undiscounted value is **kept as a labelled sensitivity row** on the `Valuation` sheet ("Sensitivity
  (not in the football field): EV / adj. EBITDA, FY2028E, UNDISCOUNTED") and as a lens row in
  `13_valuation_summary.csv` named `EV / adj. EBITDA, FY28E (undiscounted, value at end-FY2028;
  sensitivity)`. It is excluded from the low / high / mean.
- **Interim cash flows are not added to any lens.** Cash generated between 4 Sep 2026 and the value date
  enters only through the FY2027E (or FY2028E) net-cash balance on the EV lenses, and only through the
  buyback-reduced share count on the two P/ lenses and the earnings proxy.
- **The end-FY2027 arithmetic is very slightly generous as a 30 Sep 2027 label.** Straight-lining net cash
  and the share count three quarters of the way from FY2026E to FY2027E gives **$179.44** on the base
  EV/EBITDA lens against the model's **$180.88** - $1.44, or 0.8%. Not worth restating; worth knowing.

### 2. Share count: 597.0M is a weighted-average proxy for a period-end fully diluted count

**Decision.** Label it as an approximation everywhere. Specifically:

- **The 597.0M anchor is 2Q26 *diluted weighted-average* shares (SEC XBRL)** used as a proxy for the
  30 June 2026 *period-end fully diluted* share count. Those are different quantities: a weighted average
  spans the quarter and reflects average dilution over it, while a period-end fully diluted count is a
  point-in-time capitalisation including the dilutive effect of awards outstanding on that date. The
  roll-forward calls its output "diluted shares, period end", so the level inherits the anchor's basis.
- **Applying the share price to SBC dollars is an issuance proxy, not an award schedule.** The model
  issues `SBC / share price x (1 - 35% withholding)` shares a year, with the price rising 5% a year. Real
  issuance follows vesting schedules for awards granted at historical prices; no such schedule is
  disclosed, so this is the available approximation and is not calibrated to one.
- **The EPS line is an earnings proxy on modelled shares, not a reproduction of GAAP EPS.** It divides
  modelled net income by the modelled period-end share count. GAAP diluted EPS uses weighted-average
  diluted shares under the treasury-stock method. Do not present the model's EPS as a GAAP EPS forecast,
  and do not compare it to reported GAAP EPS without saying which basis each side is on. Comparing it to
  Street **adjusted** EPS (as the `Street` sheet does) carries the same caveat in the other direction.
- **What the WS18 fix did and did not do.** It removed the double-count of the 1H26 buyback and SBC
  (`SHARE_ROLL_NETS_1H26 = True`). It did **not** convert the weighted-average anchor into a point-in-time
  fully diluted count. That remains open and is a convention question, not a recurrence of the arithmetic
  bug - do not "fix" the share roll a second time.
- **Proposed, not built: a period-end fully diluted bridge.** Take the 10-Q cover-page Class A + Class B
  shares outstanding at 30 June 2026 as the basic period-end count; add unvested RSUs outstanding and
  in-the-money options from the equity-incentive footnote (both disclosed in the 10-K/10-Q); that gives a
  point-in-time fully diluted count to anchor on. Then roll it on an actual vesting schedule - the 10-K
  discloses unrecognised SBC and its weighted-average recognition period, which pins the dollar path -
  instead of SBC dollars at a modelled price. Expected to move the level by low single-digit millions of
  shares; worth doing before the deck, not worth doing to a number that is already labelled.

### 3. The FY2025 cash bridge contains two estimates and one classification artefact

The FY2025A memo column on `Cash` (anchors on the `History` sheet, "FY2025 CASH-BRIDGE ANCHORS") is not
eleven disclosed lines. Marked as follows; no forecast year depends on any of them:

| FY2025 anchor | Value | Basis |
|---|---|---|
| D&A; D&A and other add-backs; interest income; other income/(expense); change in unearned fees; capex | $91M; $161M; $705M; -$112M; $127M; -$33M | **Derived actual** - sums of the 1Q25-4Q25 quarters of `data/processed/abnb_fcf_bridge.csv`, each read from its own shareholder letter |
| Buybacks; RSU tax withholding | live sums | **Derived actual** - sums of the 2025 quarters on the `History` sheet |
| Cash taxes | -$232M | **ESTIMATE.** WS07's 1.9% of revenue x $12,241M. Not a disclosed FY2025 cash-tax line; the book provision was $626M |
| Working-capital residual | -$139M | **RECONCILIATION RESIDUAL.** Chosen so the eight lines above reproduce the reported FY2025 FCF of $4,613M exactly. It is a plug, not an observed working-capital movement |
| Interest expense | $0M | **CLASSIFICATION, NOT AN ECONOMIC ZERO.** `abnb_fcf_bridge.csv` reports zero interest expense in every quarter of 2025 because the coupon on the $2.5bn of senior notes sits inside other income/(expense) in the letters' own bridge for that period. Airbnb had borrowing cost in FY2025; this memo cell does not show it. The forecast years take interest expense from `Inputs` ($120-128M a year) and are unaffected |

### 4. The DCF fade-period input is inert in Excel and live in Python

`Inputs` row "DCF fade period (years)" is marked **LAYOUT-FIXED** by WS17 because the DCF grid on
`Valuation` section 3 is ten written-out rows: editing the cell resizes nothing and changes no Excel
output. **Keep that warning.** One thing it does not say and should: the same input **is** functional in
the Python mirror (`13_driver_model.py` passes it to `dcf()` and to `implied_growth()`), so changing the
Bear or Bull cell away from 10 would silently make the workbook and the mirror disagree and the
216-output reconciliation would start failing. Treat the cell as read-only at 10 years in all three
scenarios until someone builds a dynamic strip (WS17 open item 5).

Separately, on the quarterly regulatory drag: applying the same annual growth drag to each quarter is
**not** automatically four times the annual effect. Compare the weighted annual aggregate before calling
it a bug.
